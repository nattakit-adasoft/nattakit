                ...
                SELECT 
                    ...
                    FROM(
                        SELECT
                            ...
                            FROM TFNMCard WHERE 1=1 AND ( FTCrdStaActive = 1 AND FTCrdStaType = 1 AND FTCrdStaShift = 1 AND (CONVERT(datetime, FDCrdExpireDate) > CONVERT(datetime, GETDATE())) )
            ...
            $tSQL = "INSERT INTO TFNTCrdShiftTmp(FTBchCode, FTCshDocNo, FNCsdSeqNo, FTCrdCode, FCCtdCardBal, FTCrdStaCrd, FTCrdStaPrc, FTCrdRmk, FDCreateOn, FTCreateBy, FTSessionID)
                ...
                SELECT 
                    ...
                    FROM(
                        SELECT
                            FTCrdCode, FCCrdValue
                            FROM TFNMCard WHERE 1=1 AND ( FTCrdStaActive = 1 AND FTCrdStaType = 1 AND FTCrdStaShift = 2 )
                            AND ( FTCrdCode NOT IN (SELECT FTCrdCode FROM TFNTCrdShiftTmp WHERE FTSessionID = '$tSessionID') )
                            $tWhereCardCode $tWhereCardType
                        ) CRD";
        ...
        case 'cardShiftTopUp' : {
            ...
            $tSQL = "INSERT INTO TFNTCrdTopUpTmp(FTBchCode, FTCthDocNo, FNCtdSeqNo, FTCrdCode, FCCtdCrdTP, FTCtdStaCrd, FTCtdStaPrc, FTCtdRmk, FDCreateOn, FTCreateBy, FTSessionID)
                ...
                SELECT 
                    ...
                    CRD.FTCrdCode AS FTCrdCode,
                    ISNULL($tCtdCrdTP, 0) AS FCCtdCrdTP,
                    '1' AS FTCtdStaCrd,
                    null AS FTCtdStaPrc,
                    null AS FTCtdRmk,
                    '$tCreateOn' AS FDCreateOn,
                    ...
                    FROM(
                        SELECT
                            FTCrdCode
                            FROM TFNMCard WHERE 1=1 AND ( FTCrdStaActive = 1 AND (FTCrdStaShift = 2 OR FTCrdStaType = 2) AND ( (CONVERT(datetime, FDCrdExpireDate)) > CONVERT(datetime, GETDATE()) ) )
                ...
                SELECT 
                    ...
                    FROM(
                        SELECT
                            FTCrdCode,
                            FCCrdValue
                            FROM TFNMCard WHERE 1=1 AND ( (FCCrdValue > 0 AND FCCrdValue IS NOT NULL) AND (FTCrdStaActive = 1) AND (FTCrdStaShift = 2 OR FTCrdStaType = 2) )
                            AND ( FTCrdCode NOT IN (SELECT FTCrdCode FROM TFNTCrdTopUpTmp WHERE FTSessionID = '$tSessionID') )
                            $tWhereCardCode $tWhereCardType
                        ) CRD";
                ...
                SELECT 
                    ...
                    FROM(
                        SELECT
                            ...
                            FROM TFNMCard WHERE FTCrdCode IN ($tCvdOldCode)
                        ) OLDCRD
                LEFT JOIN (
                     SELECT FTCrdCode, FTCrdHolderID FROM TFNMCard WHERE FTCrdCode IN ($tCvdNewCode)
                ) NEWCRD
                ...
                $tSQL = "INSERT INTO TFNTCrdShiftTmp(FTBchCode, FTCshDocNo, FNCsdSeqNo, FTCrdCode, FCCtdCardBal, FTCrdStaCrd, FTCrdStaPrc, FTCrdRmk, FDCreateOn, FTCreateBy, FTSessionID)
                    ...
                    SELECT 
                        ...
                        FROM(
                            SELECT
                                FTCrdCode, FCCrdValue
                                FROM TFNMCard WHERE FTCrdCode IN ($tCrdCode)
                            ) CRD";
    ...
    switch ($ptDocType) {
        ...
        case 'cardShiftOut' : {
            ...
            $tSQL = "UPDATE TFNTCrdShiftTmp SET FTCrdStaCrd = '1', FTCrdCode = '$tCrdCode', FTCrdRmk = null, FCCtdCardBal = ISNULL(CRD.FCCrdValue, 0), FDLastUpdOn = '$tUpdateOn'
                FROM (SELECT FCCrdValue FROM TFNMCard WHERE FTCrdCode = '$tCrdCode') AS CRD 
        ...
        case 'cardShiftTopUp' : {
            $tCrdCode = $paDataSet["tCardCode"];
            $nValue = $paDataSet["nValue"];
            
            $tSQL = "UPDATE TFNTCrdTopUpTmp SET FTCtdStaCrd = '1', FTCrdCode = '$tCrdCode', FTCtdRmk = null, FCCtdCrdTP = ISNULL($nValue, 0), FDLastUpdOn = '$tUpdateOn'
        ...
        case 'cardShiftRefund' : {
            $tCrdCode = $paDataSet["tCardCode"];
            $tCrdValue = $paDataSet["tCardValue"];
        ...
        case 'cardShiftStatus' : {
            ...
            $tSQL = "UPDATE TFNTCrdVoidTmp SET FTCvdStaCrd = '1', FTCvdOldCode = '$tCvdCode', FTCvdRmk = null, FCCvdOldBal = ISNULL(CRD.FCCrdValue, 0), FDLastUpdOn = '$tUpdateOn'
                    FROM (SELECT FCCrdValue FROM TFNMCard WHERE FTCrdCode = '$tCvdCode') AS CRD 
        ...
        case 'cardShiftChange' : {
            ...
            $tSQL = "UPDATE TFNTCrdVoidTmp SET FTCvdStaCrd = '1', FTCvdOldCode = '$tCvdOldCode', FTCvdRmk = null, FTCvdNewCode = '$tCvdNewCode', FTRsnCode = '$tRsnCode', FCCvdOldBal = ISNULL(CRD.FCCrdValue, 0), FDLastUpdOn = '$tUpdateOn'
                    FROM (SELECT FCCrdValue FROM TFNMCard WHERE FTCrdCode = '$tCvdOldCode') AS CRD 
                    WHERE FNCvdSeqNo = $pnSeq AND FTSessionID = '$tSessionID'";
