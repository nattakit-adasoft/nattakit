...
function FSnHCrdShiftChkCrdCodeFoundInDB($paParams){
    ...
    $tErrorNotFoundCardCode  = language('common/docvalidate','tErrorNotFoundCardCode');   // Add document validate (Bell) 08/01/2019
...
function FSnHCrdShiftChkCrdCodeNotDupTemp($paParams){
    ...
    $tErrorCrdTmpDup  = language('common/docvalidate','tErrorCrdTmpDup');  // Add validate for lang (Jame) 08/01/2562
...
function FSnHCrdShiftChkStaShiftInCard($paParams){
    ...
    if(!empty($bStaCardShift) && $bStaCardShift === TRUE){
        ...
        $tErrorStaCardShift = language('common/docvalidate','tErrorStaCardShift');  // Add document validate (jame) 09/01/2019
    }else if(!empty($bStaCardShift) && $bStaCardShift === FALSE){
        $tWhereStaCrdShift  = " AND CRD.FTCrdStaShift = 2 ";
        $tErrorStaCardShift = language('common/docvalidate','tErrorStaCrdShiftWithdrawned');  // Add document validate (jame) 09/01/2019
    }else{
        $tWhereStaCrdShift  = "";
    }
...
function FSnHCrdShiftChkStaActiveInCard($paParams){
    ...
    switch ($nCrdStaActive) {
        case '1':
            ...
            $tErrorStaCrdActive  = language('common/docvalidate','tErrorStaCrdActive'); // Add document validate (Jame) 09/01/2019
            break;
        case '2':
            //Status InActives
            $tWhereCrdStaActive  = " AND CRD.FTCrdStaActive = 2";
            $tErrorStaCrdActive  = language('common/docvalidate','tErrorStaCrdInActive'); // Add document validate (Jame) 09/01/2019
            break;
        case '3':
            //Status Cancle
            $tWhereCrdStaActive  = " AND CRD.FTCrdStaActive = 3";
            $tErrorStaCrdActive  = language('common/docvalidate','tErrorStaCrdCancle'); // Add document validate (Jame) 09/01/2019
            break;
        default:
            $tWhereCrdStaActive  =  "";
    }
...
function FSnHCrdShiftChkCardExpireDate($paParams){
    ...
    $tErrorCardExpire = language('common/docvalidate','tErrorCardExpire'); 
    $tSQL   =   "   UPDATE TFNTCrdShiftTmp SET FTCrdStaCrd = 2 , FTCrdRmk = '".$tErrorCardExpire."'
                    WHERE FTCrdStaCrd
                    IN(
                        SELECT CST.FTCrdCode
                        FROM TFNTCrdShiftTmp CST
                        INNER JOIN TFNMCard CRD ON CRD.FTCrdCode =  CST.FTCrdCode AND  CONVERT(VARCHAR(10),CRD.FDCrdExpireDate,121) < CONVERT(VARCHAR(10),GETDATE (),121)
