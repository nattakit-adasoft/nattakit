...
class Area_controller extends MX_Controller {
    public function __construct(){
        parent::__construct ();
        $this->load->model('address/area/Area_model');
    }
    ...
    public function FSvCAREDataList(){
        try{
            ...
            $aAreDataList   = $this->Area_model->FSaMAREList($aData); 
    ...
    public function FSvCAREEditPage(){
        try{
            ...
            $aAreData       = $this->Area_model->FSaMAREGetDataByID($aData);
            $aDataAre   = array(
                'nStaAddOrEdit' => 1,
                'aAreData'      => $aAreData
            );
    ...
    public function FSoCAREAddEvent(){ 
        try{
            ...
            $oCountDup      = $this->Area_model->FSnMARECheckDuplicate($aDataAre['FTAreCode']);
            $nStaDup        = $oCountDup['counts'];
            if($oCountDup !== FALSE && $nStaDup == 0){
                $this->db->trans_begin();
                $aStaAreMaster  = $this->Area_model->FSaMAREAddUpdateMaster($aDataAre);
                $aStaAreLang    = $this->Area_model->FSaMAREAddUpdateLang($aDataAre);
    ...
    public function FSoCAREDeleteEvent(){
        ...
        $aResDel    = $this->Area_model->FSaMAREDelAll($aDataMaster);
        $nNumRowAreLoc  = $this->Area_model->FSnMLOCGetAllNumRow();
