...
class Company_controller extends MX_Controller {
	...
	public function FSoCCMPListPage(){
		try{
			...
			$tAPIReq    = "";
			$tMethodReq = "GET";
			$aResList	= $this->Company_model->FSaMCMPList($tAPIReq,$tMethodReq,$aData);
			...
			if(isset($tBchCode) && !empty($tBchCode)){
				...
				$aCmpAddressData	= $this->Company_model->FSaMCMPSelectAddressList($aDataAddrWhere);
			}
	...
	public function FSoCCMPEditPage(){
		try{
			...
			$aVatList		= FCNoHCallVatlist();
			$aResultVatList = $this->Company_model->FSoMCMPGetVatRate($tVatCode);
	...
	public function FSvCMPGetName(){
		...
		$aResList = $this->Company_model->FSaMCMPGetName($aData);
	...
	public function FSoCMPCallAddress(){
		try{
			...
			$aDataConfigView	= [
				'aDataAddress' => $this->Company_model->FSaMGetDataAddress($aDataAddrWhere)
			];
