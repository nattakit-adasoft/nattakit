...
class Smartlockersize_controller extends MX_Controller {

    public function __construct() {
        parent::__construct ();
        $this->load->model('company/shop/Shop_model');
        $this->load->model('company/smartlockersize/Smartlockersize_model');
    ...
    public function FSvCMSDataList(){
        ...
        $aResList           = $this->Smartlockersize_model->FSaMSMSDataList($aData);
        $aGenTable          = array(
            'aDataList' 	    => $aResList,
            'nPage'     	    => $nPage,
    ...
    public function FSvCSMSPageEdit(){
        ...
        $aResult        = $this->Smartlockersize_model->FSaMSMSCheckID($aData);
    ...
    public function FSaCSMSAddEvent(){
        try{
            ...
            $oCountDup  = $this->Smartlockersize_model->FSoMSMSCheckDuplicate($aDataMaster['FTPzeCode']);
            $nStaDup    = $oCountDup[0]->counts;
            if($nStaDup == 0){
                $this->db->trans_begin();
                    $aStaSmsMaster  = $this->Smartlockersize_model->FSaMSMSAddUpdateMaster($aDataMaster);
                    $aStaSmsLang    = $this->Smartlockersize_model->FSaMSMSAddUpdateLang($aDataMaster);
    ...
    public function FSaCSMSDeleteEvent(){
        ...
        $aResDel        = $this->Smartlockersize_model->FSnMSMSDel($aDataMaster);
        $nNumRowSMSPUN  = $this->Smartlockersize_model->FSnMSMSGetAllNumRow();
        $aDeleteImage = array(
                'tModuleName'  => 'company',
                'tImgFolder'   => 'smartlockerSize',
                'tImgRefID'    => $tIDCode,
