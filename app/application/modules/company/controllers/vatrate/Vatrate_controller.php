...
class Vatrate_controller extends MX_Controller {
    ...
    */
    ...
    public function FSxVATCreateOrUpdate(){
        ...
        $this->db->trans_begin();
        $this->Vatrate_model->FSoMVATDelete($tAddVatCode);

        $aData = json_decode($tData);
        foreach ($aData as $oData){
            $this->Vatrate_model->FSoMVATCreateOrUpdate($tAddVatCode, $oData);
        }
    ...
    */
    public function FStVATUniqueValidate($tSelect = ''){
        ...
        if($this->input->is_ajax_request()){ // Request check
            if($tSelect == 'vatcode'){
                ...
                $oVatrate = $this->Vatrate_model->FSoMVATGetByVCode($tAddVatCode);
            ...
            if($tSelect == 'vatrate-vatstart'){
                ...
                $oVatrate = $this->Vatrate_model->FSoMVATGetByVCodeVRateVStart($tAddVatCode, $tAddVatRate, $tAddVatStart);
    ...
    public function FSvVATDataList(){
        ...
        $aResList   = $this->Vatrate_model->FSaMVATList($aData);
    ...
    public function FSvVATEditPage(){
        ...
        $aResList = $this->Vatrate_model->FSaMVATSearchByID($aData); // vat rate by vat code
    ...
    public function FSoVATDeleteMultiVat(){
        ...
        foreach($aVatCode as $oVatCode){
            $this->Vatrate_model->FSoMVATDelete($oVatCode);
        }
    ...
    public function FSoVATDelete(){
        ...
        $tVatCode = $this->input->post('tVatCode');
        $this->Vatrate_model->FSoMVATDelete($tVatCode);
    ...
    public function FSoVatChackDup(){
        ...
        $oCountVat  = $this->Vatrate_model->FSoMVATChkDup($dDateStart);
