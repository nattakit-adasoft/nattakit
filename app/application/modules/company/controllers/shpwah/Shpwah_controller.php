...
class Shpwah_controller extends MX_Controller {

    public function __construct() {
        parent::__construct ();
        $this->load->model('company/branch/Branch_model');
        $this->load->model('authen/user/User_model');
        $this->load->model('company/shpwah/Shpwah_model');
    ...
    */
    public function FSvCShpWahDataList(){
        ...
        $aResList   = $this->Shpwah_model->FSaMShpWahDataList($aData);
    ...
    */
    ...
    public function FSaCShpWahAddEvent(){
        ...
        $aChkDupicate    = $this->Shpwah_model->FSaMShpWahCheckCode($aDataMaster);
        ...
        if($aChkDupicate['rtCode'] == 1){
            ...
            //Insert Data
            $this->db->trans_begin();
            $this->Shpwah_model->FSaMRShpWahAddMaster($aDataMaster);
            if($this->db->trans_status() === false){
                ...
                $this->Shpwah_model->FSaMRShpWahCheckWahCodeINShop('ADD',$aDataMaster);
            }
        }
        unset($aDataMaster);
        unset($aChkDupicate);
    ...
    public function FSaCShpWahDeleteEvent(){
        ...
        $aResDel    = $this->Shpwah_model->FSnMUShpWahDel($aDataDeleteMaster);
        $nNumRowRsnLoc  = $this->Shpwah_model->FSnMLOCGetAllNumRow();
        if($nNumRowRsnLoc){

            //เดียวจะวิ่งไปเช็คก่อนว่าในร้านค้า นั้น มี wah หรือยัง ถ้ายังไม่มีต้องเอาไป update
            $this->Shpwah_model->FSaMRShpWahCheckWahCodeINShop('DEL',$aDataDeleteMaster);
    ...
    public function FSoCShpWahDelMultipleEvent(){
        try{
           ...
           $tResult = $this->Shpwah_model->FSaMShpWahDeleteMultiple($aDataDelMulti);
           ...
           if($this->db->trans_status() == FALSE){
            ...
            $aDataReturn    = array(
                ...
                //เดียวจะวิ่งไปเช็คก่อนว่าในร้านค้า นั้น มี wah หรือยัง ถ้ายังไม่มีต้องเอาไป update
                $this->Shpwah_model->FSaMRShpWahCheckWahCodeINShop('DELMULTI',$aDataDelMulti);
