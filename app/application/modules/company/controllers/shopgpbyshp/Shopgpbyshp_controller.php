...
class Shopgpbyshp_controller extends MX_Controller {

    public function __construct() {
        parent::__construct ();
        $this->load->model('company/shop/Shop_model');
        $this->load->model('company/shopgpbyshp/Shopgpbyshp_model');
    }
    ...
    public function FSvCShopGpByShpDataList(){
        try{
            ...
            $aResList           = $this->Shopgpbyshp_model->FSaMShopGpByShpDataList($aData);
    ...
    public function FSvCShopGpByShpAdd(){
        ...
        try{
            ...
            $nRunSeq  = $this->Shopgpbyshp_model->FSaMShopGpFindSeq($tBchCode,$tShpCode);
           ...
           //เช็ควันที่ซ้ำหรือไม่
            $aCheckData      = $this->Shopgpbyshp_model->FSaMShopGpByShpCheckeData($aDataMaster);
            if($aCheckData['rtCode'] == '1'){
                ...
                $aStaDelete  =  $this->Shopgpbyshp_model->FSaMShopGpByShpDateDeleteData($aDataMaster);
                if($aStaDelete['rtCode'] == '1'){
                    $aStaAddUpdateMaster  =  $this->Shopgpbyshp_model->FSaMSHPAddUpdateMaster($aDataMaster);
                        $aStatus = array(
    ...
    public function FSvCShopGpByShpEdit(){
        ...
        try{
            ...
            if($dDateNew == $dDateOld){
                $aStaAddUpdateMaster  =  $this->Shopgpbyshp_model->FSaMSHPEditUpdateMaster($aDataMaster);
                $aStatus = array(
                    'nStaCallBack'	=> $this->session->userdata('tBtnSaveStaActive'),
                    'nStaEvent'	    => '1',
    ...
    public function FSvCShopGpByShpDel(){
        if ($this->input->post('tIDCode')) {
            ...
            $aResDel                = $this->Shopgpbyshp_model->FSaMSHPDelAll($aData);
            $nNumRowPdtGpbyShop     = $this->Shopgpbyshp_model->FSnMMSHPGetAllNumRow();
            //เช็ด เปลี่ยนหน้า 
    ...
    public function FSvCShopGpByEditinLinePageShop(){
        try{
            ...
            $aShpShopData  = $this->Shopgpbyshp_model->FSaMSHPCheckDatabeforeUpdate($aData);
            if(isset($aShpShopData)){
                $aStatus   = $this->Shopgpbyshp_model->FSaMSHPUpdateDataInline($aData);
                echo json_encode($aStatus);
            }
    ...
    public function FSvCShopGpByShpPageAdd(){
        ...
        //Get ชื่อร้านค้า
        $aDT                = $this->Shopgpbyshp_model->FSaMShopGpBySelectDataDT($aData,'shop');
        ...
        if($tSesUserLevel == 'HQ'){
            ...
            if (strpos($tBchCode, ',') !== false) {
                ...
                $tOptionBrowseBch   = 0;
                $tWhereBranch       = '';
                $tNameBch           = $this->Shopgpbyshp_model->FSaMShopGpBySelectDataDT($aData,'branch');
            }
        }else{
            $tOptionBrowseBch   = 0;
            $tWhereBranch       = '';
    ...
    function FSvCSHPEditPage(){
        ...
        $vBtnSave		= FCNaHBtnSaveActiveHTML('shop/0/0');
        $aResult        = $this->Shopgpbyshp_model->FSaMShopGpGetData($aData);
        $aDataEdit      = array(
                                'aResult'                => $aResult,
    ...
    public function FSvCShopGpByShpEventCheckData(){
        ...
        $tResult = $this->Shopgpbyshp_model->FSaMShopGpByShpCheckeData($aGetGPShop);
        $aResList   = array(
            'rtCode'      => '1',
    ...
    public function FSvCShopGpByShpEventInsertGP(){
            ...
            $this->Shopgpbyshp_model->FSaMSHPGPEditUpdateMaster($aDataMaster);
        }
    }