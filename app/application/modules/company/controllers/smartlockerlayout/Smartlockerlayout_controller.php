...
class Smartlockerlayout_controller extends MX_Controller {


    public function __construct() {
        parent::__construct ();
        $this->load->model('company/smartlockerlayout/Smartlockerlayout_model');
    ...
    public function FSvCSMLMainPage(){
        ...
        $tGetLayoutGroup    = $this->Smartlockerlayout_model->FSaMSMLGetGroup($tBchCode,$tShpCode,$nLangEdit);
        $tGetLayoutColumn   = $this->Smartlockerlayout_model->FSaMSMLGetColumn($tBchCode,$tShpCode);
    ...
    public function FSvCSMLDataList(){
        ...
        $aResList           = $this->Smartlockerlayout_model->FSaMSMLDataList($aData);
    ...
    public function FSvCSMLInsert(){
        ...
        $tCheckLayno = $this->Smartlockerlayout_model->FSaMSMLCheckLayout($aData);
       
        if($tCheckLayno['rtCode'] == '1'){
            $tCheckColumnAndFloor = $this->Smartlockerlayout_model->FSaMSMLCheckColumnANDFloor($aData,'ADD');
            if($tCheckColumnAndFloor['rtCode'] == '1' ){
                //Insert
                $this->Smartlockerlayout_model->FSaMSMLInsertLayout($aData);
            }else{
    ...
    public function FSvCSMLDelete(){
        ...
        $this->Smartlockerlayout_model->FSaMSMLDelete($aData);
    ...
    public function FSvCSMLEdit(){
        ...
        if($tBch == $tSMLOldBch && $tSMLOldShp == $tSMLOldShp &&  $tSMLOldLayno == $tLayno){
            $tCheckColumnAndFloor = $this->Smartlockerlayout_model->FSaMSMLCheckColumnANDFloor($aData,$aWhere);
            if($tCheckColumnAndFloor['rtCode'] == '1' ){
               //update
                $this->Smartlockerlayout_model->FSaMSMLUpdateLayout($aData,$aWhere);
            }else{
                //2 คอลั่ม และ ชั่น ซ้ำ
                echo 2;
            }
