...
class Pdtgroup_controller extends MX_Controller {

    public function __construct(){
        parent::__construct ();
        $this->load->model('product/pdtgroup/Pdtgroup_model');
    ...
    public function FSvCPGPDataList(){
        try{
            ...
            $aPgpDataList           = $this->Pdtgroup_model->FSaMPGPList($aData);
            $aAlwEventPdtGroup	    = FCNaHCheckAlwFunc('pdtgroup/0/0');
            $aGenTable  = array(
    ...
    public function FSvCPGPEditPage(){
        try{
            ...
            $aResult       = $this->Pdtgroup_model->FSaMPGPGetDataByID($aData);
    ...
    public function FSoCPGPAddEvent(){
        try{
            ...
            if($tSelectRoot == 'on'){
                ...
                $aDataPgpSelect     = $this->Pdtgroup_model->FSaMPGPGetDataParent($tPgpParent,$nLngID);
            ...
            $this->db->trans_begin();
            $aStaPgpMaster  = $this->Pdtgroup_model->FSaMPGPAddUpdateMaster($aDataPdtGroup);
            $aStaPgpLang    = $this->Pdtgroup_model->FSaMPGPAddUpdateLang($aDataPdtGroup);
    ...
    public function FSoCPGPDeleteEvent(){
        try{
            ...
            $aDataDelete = $this->Pdtgroup_model->FSaMGetChainForDelete($aDataMaster);
            $aResDel    = $this->Pdtgroup_model->FSaMPGPDelAll($aDataDelete);
            $nNumRowPdtPGP = $this->Pdtgroup_model->FSnMPGPGetAllNumRow();
            $aDeleteImage = array(
                'tModuleName'  => 'product',
                'tImgFolder'   => 'pdtgroup',
                'tImgRefID'    => $tIDCode  ,
