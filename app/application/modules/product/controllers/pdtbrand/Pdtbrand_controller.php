...
class Pdtbrand_controller extends MX_Controller {
    public function __construct(){
        parent::__construct ();
        $this->load->model('product/pdtbrand/Pdtbrand_model');
    ...
    public function FSvCBNDataList(){
        try{
            ...
            $aPbnDataList           = $this->Pdtbrand_model->FSaMBNList($aData);
            $aAlwEventPdtBrand	    = FCNaHCheckAlwFunc('pdtbrand/0/0');
            $aGenTable  = array(
    ...
    public function FSvCBNEditPage(){
        try{
            ...
            $aPbnData       = $this->Pdtbrand_model->FSaMBNGetDataByID($aData);
            $aDataPdtBrand   = array(
                'nStaAddOrEdit' => 1,
                'aPbnData'      => $aPbnData
            );
    ...
    public function FSoCBNAddEvent(){
        try{
            ...
            $this->db->trans_begin();
            $aStaPbnMaster  = $this->Pdtbrand_model->FSaMBNAddUpdateMaster($aDataPdtBrand);
            $aStaPbnLang    = $this->Pdtbrand_model->FSaMBNAddUpdateLang($aDataPdtBrand);
    ...
    public function FSoCBNDeleteEvent(){
        ...
        $aResDel    = $this->Pdtbrand_model->FSaMBNDelAll($aDataMaster);
        $nNumRowPbn = $this->Pdtbrand_model->FSnMPGPGetAllNumRow();
        if($nNumRowPbn!==false){
            $aReturn    = array(
