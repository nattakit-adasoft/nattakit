...
class Pdtlocation_controller extends MX_Controller {
    public function __construct(){
        parent::__construct ();
        $this->load->model('product/pdtlocation/Pdtlocation_model');
    ...
    public function FSvCLOCDataList(){ 
        try{
            ...
            $aLocDataList               = $this->Pdtlocation_model->FSaMLOCList($aData); 
            $aAlwEventPdtLocation	    = FCNaHCheckAlwFunc('pdtlocation/0/0');
            $aGenTable  = array(
    ...
    public function FSvCLOCSeqDataList(){
        try{
            ...
            $aLocDataList   = $this->Pdtlocation_model->FSaMLOCGetDataLocSeqByID($aData);
            $aGenTable  = array(
    ...
    public function FSvCLOCEditPage(){  
        try{
            ...
            $aLocData       = $this->Pdtlocation_model->FSaMLOCGetDataByID($aData);
            $aDataPdtLoc   = array(
                'nStaAddOrEdit' => 1,
                'aLocData'      => $aLocData
            );
    ...
    public function FSvCLOCManagePage(){  
        try{
            ...
            $aLocSeqData    = $this->Pdtlocation_model->FSaMLOCGetDataLocSeqByID($aDataLocSeq);
            if($aLocSeqData['rtCode']=='1' && $aLocSeqData['raItems'][0]['rtLastUpdBy']==""){
                $aLocSeqDel = $this->Pdtlocation_model->FSaMLOCSeqDeleteDataAllByID($aLocSeqData['raItems'][0]['rtPlcCode']);
            }
    ...
    public function FSoCLOCAddEvent(){  
        try{
            ...
            $oCountDup      = $this->Pdtlocation_model->FSnMLOCCheckDuplicate($aDataPdtLoc['FTPlcCode']);
            $nStaDup        = $oCountDup['counts'];
            if($oCountDup !== FALSE && $nStaDup == 0){
                $this->db->trans_begin();
                $aStaLocMaster  = $this->Pdtlocation_model->FSaMLOCAddUpdateMaster($aDataPdtLoc);
                $aStaLocLang    = $this->Pdtlocation_model->FSaMLOCAddUpdateLang($aDataPdtLoc);
    ...
    public function FSoCLOCDeleteEvent(){ 
        ...
        $aResDel    = $this->Pdtlocation_model->FSaMLOCDelAll($aDataMaster);
        $nStaLocSeq = $this->Pdtlocation_model->FSaMLOCSeqDeleteDataAllByID($tPlcCode);
        $nNumRowPdtLoc = $this->Pdtlocation_model->FSnMLOCGetAllNumRow();
    ...
    public function FSvCLOCGetDataPdtGrp(){
        try{
            ...
            $aPgpData   = $this->Pdtlocation_model->FSaMLOCGetDataPdtGrpByID($aData);
    ...
    public function FSvCLOCGetDataPdtTyp(){
        try{
            ...
            $aPtyData   = $this->Pdtlocation_model->FSaMLOCGetDataPdtTypByID($aData);
    ...
    public function FSoCLOCManageAddEvent($paData){
        if($paData['rtCode']=="1"){
            ...
            for($i=0;$i<$nCount;$i++){
                ...
                $oCountDup  = $this->Pdtlocation_model->FSnMLOCSeqCheckDuplicate($aData);
                $nStaDup    = $oCountDup['counts'];
                if($oCountDup !== FALSE && $nStaDup == 0){
                    $nData = $this->Pdtlocation_model->FSaMLOCLocSeqAddData($aData);
    ...
    public function FSoCLOCManageEditEvent(){
        try{
            ...
            for($i=0;$i<$nNumProduct;$i++){
                ...
                $aLocSeqData = $this->Pdtlocation_model->FSaMLOCEditDataLocSeq($aData);
    ...
    public function FSoCLOCSeqDeleteEvent(){ 
        ...
        $aResDel    = $this->Pdtlocation_model->FSaMLOCSeqDelAll($aDataMaster);
        $aReturn    = array(
            'nStaEvent' => $aResDel['rtCode'],
            'tStaMessg' => $aResDel['rtDesc']
        );
