...
class Pdtunit_controller extends MX_Controller {

    public function __construct(){
        parent::__construct ();
        $this->load->model('product/pdtunit/Pdtunit_model');
    ...
    public function FSvCPUNDataList(){
        try{
            ...
            $aPunDataList           = $this->Pdtunit_model->FSaMPUNList($aData);
            $aAlwEventPdtUnit	    = FCNaHCheckAlwFunc('pdtunit/0/0');
            $aGenTable  = array(
    ...
    public function FSvCPUNEditPage(){
        try{
            ...
            $aPunData       = $this->Pdtunit_model->FSaMPUNGetDataByID($aData);
            $aDataPdtUnit      = array(
                'nStaAddOrEdit' => 1,
                'aPunData'      => $aPunData
            );
    ...
    public function FSoCPUNAddEvent(){
        try{
            ...
            $oCountDup      = $this->Pdtunit_model->FSnMPUNCheckDuplicate($aDataPdtUnit['FTPunCode']);
            $nStaDup        = $oCountDup['counts'];
            if($oCountDup !== FALSE && $nStaDup == 0){
                $this->db->trans_begin();
                $aStaDptMaster  = $this->Pdtunit_model->FSaMPUNAddUpdateMaster($aDataPdtUnit);
                $aStaDptLang    = $this->Pdtunit_model->FSaMPUNAddUpdateLang($aDataPdtUnit);
    ...
    public function FSoCPUNEditEvent(){
        try{
            ...
            $this->db->trans_begin();
            $aStaPunMaster  = $this->Pdtunit_model->FSaMPUNAddUpdateMaster($aDataPdtUnit);
            $aStaPunLang    = $this->Pdtunit_model->FSaMPUNAddUpdateLang($aDataPdtUnit);
            if($this->db->trans_status() === FALSE){
                $this->db->trans_rollback();
                $aReturn = array(
                    'nStaEvent'    => '900',
    ...
    public function FSoCPUNDeleteEvent(){
        ...
        $aResDel        = $this->Pdtunit_model->FSaMPUNDelAll($aDataMaster);
        $nNumRowPdtPUN = $this->Pdtunit_model->FSnMPUNGetAllNumRow();
