...
class Pdttouchgroup_controller extends MX_Controller {
    ...
    */
    ...
    public function __construct() {
        $this->FSxCInitParams();
        $this->load->model('product/pdttouchgroup/Pdttouchgroup_model');
        parent::__construct();
    }
    ...
    public function FSvCTCGCallPageDataTable() {
        try{
            ...
            $aDataList  = $this->Pdttouchgroup_model->FSaMTCGDataTableList($aDataCondition);
            $aConfigView    = [
                'nPage'     => $nPage,
                'aAlwEvent' => $this->aAlwEvent,
                'aTextLang' => $this->aTextLang,
    ...
    public function FSvCTCGCallPageEdit(){
        ...
        $aDataResult    = $this->Pdttouchgroup_model->FSaMTCGGetDataByID($aDataWhere);
    ...
    public function FSoCTCGEventAdd(){
        ...
        $this->Pdttouchgroup_model->FSaMTCGAddDataMaster($aDataMaster);
    ...
    public function FSoCTCGEventEdit(){
        ...
        $this->Pdttouchgroup_model->FSaMTCGUpdateDataMaster($aDataMaster);
        if($this->db->trans_status() === false){
            $this->db->trans_rollback();
            $aReturn = array(
                'nStaEvent' => '900',
    ...
    public function FSoCTCGEventDelete(){
        try{
            ...
            $aStaDelete     = $this->Pdttouchgroup_model->FSnMTCGEventDelete($aDataWhere);
