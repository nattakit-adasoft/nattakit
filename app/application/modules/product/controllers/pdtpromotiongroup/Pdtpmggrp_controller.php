...
class Pdtpmggrp_controller extends MX_Controller {

    public function __construct(){
        parent::__construct ();
        $this->load->model('product/pdtpromotiongroup/Pdtpmggrp_model');
    }
    ...
    public function FSvCPMGDataList(){
        try{
            ...
            $aPmgDataList               = $this->Pdtpmggrp_model->FSaMPMGList($aData);
            $aAlwEventPdtPromotion	    = FCNaHCheckAlwFunc('pdtpmggroup/0/0');
            $aGenTable  = array(
    ...
    public function FSvCPMGEditPage(){
        try{
            ...
            $aPmgData       = $this->Pdtpmggrp_model->FSaMPMGGetDataByID($aData);
            $aDataPdtPmgGrp   = array(
                'nStaAddOrEdit' => 1,
                'aPmgData'      => $aPmgData
            );
    ...
    public function FSoCPMGAddEvent(){
        try{
            ...
            $this->db->trans_begin();
            $aStaPmgMaster  = $this->Pdtpmggrp_model->FSaMPMGAddUpdateMaster($aDataPdtPmgGrp);
            $aStaPmgLang    = $this->Pdtpmggrp_model->FSaMPMGAddUpdateLang($aDataPdtPmgGrp);
    ...
    public function FSoCPMGDeleteEvent(){
        ...
        $aResDel    = $this->Pdtpmggrp_model->FSaMPMGDelAll($aDataMaster);
        $nNumRowPdtPmg = $this->Pdtpmggrp_model->FSnMPMGGetAllNumRow();
