...
class Pdttype_controller extends MX_Controller {

    public function __construct(){
        parent::__construct ();
        $this->load->model('product/pdttype/Pdttype_model');
    ...
    public function FSvCPTYDataList(){
        try{
            ...
            $aPtyDataList           = $this->Pdttype_model->FSaMPTYList($aData);
            $aAlwEventPdtType	    = FCNaHCheckAlwFunc('pdttype/0/0');
            $aGenTable  = array(
    ...
    public function FSvCPTYEditPage(){
        try{
            ...
            $aPtyData       = $this->Pdttype_model->FSaMPTYGetDataByID($aData);
            $aDataPdtType   = array(
                'nStaAddOrEdit' => 1,
                'aPtyData'      => $aPtyData
            );
    ...
    public function FSoCPTYAddEvent(){
        try{
            ...
            $this->db->trans_begin();
            $aStaPtyMaster  = $this->Pdttype_model->FSaMPTYAddUpdateMaster($aDataPdtType);
            $aStaPtyLang    = $this->Pdttype_model->FSaMPTYAddUpdateLang($aDataPdtType);
    ...
    public function FSoCPTYDeleteEvent(){
        ...
        $aResDel    = $this->Pdttype_model->FSaMPTYDelAll($aDataMaster);
        $nNumRowPdtPty = $this->Pdttype_model->FSnMLOCGetAllNumRow();
