...
class Merpdtgroup_controller extends MX_Controller {

    public function __construct(){
        parent::__construct ();
        $this->load->model('product/merpdtgroup/Merpdtgroup_model');
    }
    ...
    public function FSvCMgpDataList(){
        try{
            ...
            $aMgpDataList           = $this->Merpdtgroup_model->FSaMMGPList($aData);
    ...
    public function FSvCMGPEditPage(){
        try{
            ...
            $aResult       = $this->Merpdtgroup_model->FSaMMGPGetDataByID($aData);
            $aDataEdit     = array(
            'aResult'           => $aResult,
    ...
    public function FSoCMGPAddEvent(){
        try{
            ...
            //ตรวจสอบบค่าซ้ำ
            $oCountDup  = $this->Merpdtgroup_model->FSnMMGPCheckDuplicate($aDataMaster['FTMgpCode']);
            $nStaDup    = $oCountDup[0]->counts;

            if($nStaDup == 0){
                $this->db->trans_begin();
                $aStaEventMaster  = $this->Merpdtgroup_model->FSaMMGPAddUpdateMaster($aDataMaster);
                $aStaEventLang    = $this->Merpdtgroup_model->FSaMMGPAddUpdateLang($aDataMaster);
    ...
    public function FSoCMgpDeleteEvent(){
        try{
            ...
            $aDataDelete = $this->Merpdtgroup_model->FSaMGetChainForDelete($aDataMaster);
            $aResDel    = $this->Merpdtgroup_model->FSaMMgpDelAll($aDataDelete);
            $nNumRowPdtPGP = $this->Merpdtgroup_model->FSnMPGPGetAllNumRow();
