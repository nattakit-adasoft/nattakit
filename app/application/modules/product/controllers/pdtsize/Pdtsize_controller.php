...
class Pdtsize_controller extends MX_Controller {
    public function __construct(){
        parent::__construct ();
        $this->load->model('product/pdtsize/Pdtsize_model');
    ...
    public function FSvCPSZDataList(){ 
        try{
            ...
            $aPszDataList           = $this->Pdtsize_model->FSaMPSZList($aData); 
            $aAlwEventPdtSize	    = FCNaHCheckAlwFunc('pdtsize/0/0');
            $aGenTable  = array(
    ...
    public function FSvCPSZEditPage(){  
        try{
            ...
            $aPszData       = $this->Pdtsize_model->FSaMPSZGetDataByID($aData);
            $aDataPdtSize   = array(
                'nStaAddOrEdit' => 1,
                'aPszData'      => $aPszData
            );
    ...
    public function FSoCPSZAddEvent(){  
        try{
            ...
            $oCountDup      = $this->Pdtsize_model->FSnMPSZCheckDuplicate($aDataPdtSize['FTPszCode']);
            $nStaDup        = $oCountDup['counts'];
            if($oCountDup !== FALSE && $nStaDup == 0){
                $this->db->trans_begin();
                $aStaPszMaster  = $this->Pdtsize_model->FSaMPSZAddUpdateMaster($aDataPdtSize);
                $aStaPszLang    = $this->Pdtsize_model->FSaMPSZAddUpdateLang($aDataPdtSize);
    ...
    public function FSoCPSZDeleteEvent(){ 
        ...
        $aResDel    = $this->Pdtsize_model->FSaMPSZDelAll($aDataMaster);
        $nNumRowPsz = $this->Pdtsize_model->FSnMPGPGetAllNumRow();
        if($nNumRowPsz!==false){
            $aReturn    = array(
