...
class Vendingshoptype_controller extends MX_Controller {
    
    public function __construct() {
        parent::__construct ();
        $this->load->model('vending/vendingshoptype/Vendingshoptype_model');
    }
    ...
    public function FSvVSTDataList(){
        ...
        $tAPIReq    = "";
        $tMethodReq = "GET";
        $aResList   = $this->Vendingshoptype_model->FSaMVstList($tAPIReq,$tMethodReq,$aData);
    ...
    public function FSvVSTEditPage(){
        ...
        $tAPIReq        = "";
        $tMethodReq     = "GET";
        $aPackData      = $this->Vendingshoptype_model->FSaMVstSearchByID($tAPIReq,$tMethodReq,$aData);
        $aDataEdit = array(
            'tFTBchCode'    => $tBchCode,
            'tFTShpCode'    => $tShpCode,
            'aResult'       => $aPackData
    ...
    public function FSaVSTAddEvent(){
        try{
            ...
            if($tTypepage == 'Add'){
                ...
                $oCountDup  = $this->Vendingshoptype_model->FSoMVstCheckDuplicate($aDataMaster['FTShtCode']);
                $nStaDup    = $oCountDup[0]->counts;

                if(trim($nStaDup) == 0){
                    $this->db->trans_begin();
                    $aStaVstMaster  = $this->Vendingshoptype_model->FSaMVstAddUpdateMaster($aDataMaster,$tTypepage);
                    $aStaVstLang    = $this->Vendingshoptype_model->FSaMVstAddUpdateLang($aDataMaster,$tTypepage);
    ...
    public function FSaVSTDeleteEvent(){
        ...
        // $tMethodReq     = 'POST';
        // $aResDel        = $this->Vendingshoptype_model->FSnMVstDel($tAPIReq,$tMethodReq,$aDataMaster);
        // $aReturn    = array(
