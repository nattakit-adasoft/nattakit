...
class Vendinglayout_controller extends MX_Controller {

    public function __construct() {
        parent::__construct ();
        $this->load->model('vending/vendinglayout/Vendinglayout_model');
    ...
    public function FSvVEDSelectSetting(){
        ...
        $aGetDataHeightFloor    = $this->Vendinglayout_model->FSaMVEDGetDataHeightFloor($paPackData);
        $aGetDataHeightTemp     = $this->Vendinglayout_model->FSaMVEDGetDataHeightTemp($paPackData);
    ...
    public function FSvVEDShowViewDiagram($paPackData){
        $aGetDataCabinet    = $this->Vendinglayout_model->FSaMVEDGetCabinet($paPackData);
        $aGetWahhouse       = $this->Vendinglayout_model->FSaMVEDGetWahhouse($paPackData);
    ...
    public function FSvVEDInsertSetting(){
        ...
        $this->Vendinglayout_model->FSaMVEDDeleteHeightToTmp($aPackData);
        for($i=1; $i<=count($aHeight); $i++){
            $this->Vendinglayout_model->FSaMVEDInsertHeightToTmp($i,$aHeight[$nKeyIndex],$aPackData);
            $nKeyIndex++;
        }
        
        //เพิ่มข้อมูลลงตาราง TVDMShopCabinet (เก็บว่าใช้กี่ชั้น ใช้กี่ช่อง)
        $aFindVendingLayout     = $this->Vendinglayout_model->FSxMVEDUpdateCabinet($aPackData);
    ...
    public function FSxVEDInsertDiagram(){
        ...
        $aReturnMerCode = $this->Vendinglayout_model->FSaMVslFindMerCode($aDataCondition);

        //ลบสินค้าก่อนทุกครั้ง
        $this->Vendinglayout_model->FSaMVslDeleteItem($aDataCondition);
        ...
        for($j=0; $j<count($aPackData); $j++){
            ...
            $aDataInsert = array(
                ...
                'FCLayHigh'         => $this->Vendinglayout_model->FStMVEDFindHeight(trim($tBchCode),trim($tShopCode),$aColumn['nFloor'],$nSeqCabinet),
            ...
            $this->Vendinglayout_model->FSaMVslInsertPDT($aDataInsert);
        }

        //เรียง Seq ใหม่
        $this->Vendinglayout_model->FSxMVEDSortSeqrow($aDataCondition);
    }
    ...
    public function FSxVEDGetPDTShopLayout(){
        ...
        $aGetDataDT     = $this->Vendinglayout_model->FSaMVEDGetDataDT($paPackData);
        $aData = array(
            'aDT'       => $aGetDataDT,
        );
    ...
    public function FSxVEDDeleteDiagram(){
        ...
        $aDeleteLayout = $this->Vendinglayout_model->FSaMVEDDeleteDiagram($paPackData);
