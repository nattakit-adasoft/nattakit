...
class Saleorder_controller extends MX_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('document/saleorder/Saleorder_model');
        $this->load->model('document/saleorder/Saleorderdischgmodal_model');
        parent::__construct();
    }
    ...
    public function FSoCSODataTable() {
        try {
            ...
            $aDataList = $this->Saleorder_model->FSaMSOGetDataTableList($aDataCondition);
    ...
    public function FSoCSODeleteEventDoc() {
        try {
            ...
            $aResDelDoc = $this->Saleorder_model->FSnMSODelDocument($aDataMaster);
    ...
    public function FSoCSOAddPage() {
        try {
            ...
            if(!empty($tUserBchCode)){
                $aDataBch = $this->Saleorder_model->FSaMSOGetDetailUserBranch($tUserBchCode);
            ...
            $this->Saleorder_model->FSaMCENDeletePDTInTmp($aWhereClearTemp);
            $this->Saleorder_model->FSxMSOClearDataInDocTemp($aWhereClearTemp);
            ...
            $tASOReq = "";
            $tMethodReq = "GET";
            $aCompData = $this->Company_model->FSaMCMPList($tASOReq, $tMethodReq, $aDataWhere);
            ...
            $aDataUserGroup = $this->Saleorder_model->FSaMSOGetShpCodeForUsrLogin($aDataShp);
            ...
            $aConfigSysWareHouse = $this->Saleorder_model->FSaMSOGetDefOptionConfigWah($aConfigSys);
    ...
    public function FSoCSOEditPage() {
        ...
        try {
            ...
            $aDataDocHD = $this->Saleorder_model->FSaMSOGetDataDocHD($aDataWhere);
            ...
            $this->Saleorder_model->FSxMSOMoveHDDisToTemp($aDataWhere);

            // Move Data DT TO DTTemp
            $this->Saleorder_model->FSxMSOMoveDTToDTTemp($aDataWhere);

            // Move Data DTDIS TO DTDISTemp
            $this->Saleorder_model->FSxMSOMoveDTDisToDTDisTemp($aDataWhere);
    ...
    public function FSoCSOEditPageMonitor() {
        try {
          ...
          $nCheckNumBook =  $this->Saleorder_model->FSnMSOCheckStrPrcLastUpdate($aDataSetStrPrc);
         
           if($nCheckNumBook>0){//ตรวจสอบว่าในขณะนี้มีผู้จองเอกสารใช้อยู่หรือไม่ 0 = มีผู้จองใช้อยู่ , >0 = เอกสารว่างในขณะนี้

            $this->Saleorder_model->FSaMSOUpdateStrPrcLastUpdate($aDataSetStrPrc);
            ...
            if ($this->db->trans_status() === FALSE) {
              ...
              $nSecondTimeCountDonw = $this->Saleorder_model->FSnMSOGetTimeCountDown($aDataSetStrPrc);
    ...
    public function FSoCSOPdtAdvTblLoadData() {
        try {
            ...
            $aDataDocDTTemp = $this->Saleorder_model->FSaMSOGetDocDTTempListPage($aDataWhere);
            $aDataDocDTTempSum = $this->Saleorder_model->FSaMSOSumDocDTTemp($aDataWhere);
            // echo '<pre>';
            //         print_r($aColumnShow);
            // echo '</pre>';
            $aDataView = array(
    ...
    public function FSoCSOAddPdtIntoDocDTTemp() {
        try {
            ...
            for ($nI = 0; $nI < count($aSOPdtData); $nI++) {
                ...
                $cSOPrice = $this->Saleorder_model->FScMSOGetPricePdt4CstOrPdtBYPplCode($aDataGetprice);
                // $cSOPrice = $aSOPdtData[$nI]->packData->PriceRet;
                $nSOMaxSeqNo = $this->Saleorder_model->FSaMSOGetMaxSeqDocDTTemp($aDataWhere);
                ...
                $aDataPdtMaster = $this->Saleorder_model->FSaMSOGetDataPdt($aDataPdtParams);
                // นำรายการสินค้าเข้า DT Temp
                $nStaInsPdtToTmp = $this->Saleorder_model->FSaMSOInsertPDTToTemp($aDataPdtMaster, $aDataPdtParams);
            }
    ...
    public function FSoCSOEditPdtIntoDocDTTemp() {
        try {
            ...
            $this->Saleorder_model->FSaMSOUpdateInlineDTTemp($aDataUpdateDT, $aDataWhere);

            if ($nSOIsDelDTDis == '1') {
                // ยืนยันการลบ DTDis ส่วนลดรายการนี้
                $this->Saleorderdischgmodal_model->FSaMSODeleteDTDisTemp($aDataWhere);
                $this->Saleorderdischgmodal_model->FSaMSOClearDisChgTxtDTTemp($aDataWhere);
            }
    ...
    public function FSvCSORemovePdtInDTTmp() {
        try {
            ...
            $aStaDelPdtDocTemp = $this->Saleorder_model->FSnMSODelPdtInDTTmp($aDataWhere);
    ...
    public function FSvCSORemovePdtInDTTmpMulti() {
        try {
            ...
            $aStaDelPdtDocTemp = $this->Saleorder_model->FSnMSODelMultiPdtInDTTmp($aDataWhere);
    ...
    public function FSoCSOChkHavePdtForDocDTTemp() {
        try {
            ...
            $nCountPdtInDocDTTemp = $this->Saleorder_model->FSnMSOChkPdtInDocDTTemp($aDataWhere);
    ...
    private function FSaCSOCalDTTempForHD($paParams) {
        $aCalDTTemp = $this->Saleorder_model->FSaMSOCalInDTTemp($paParams);
    ...
    public function FSoCSOAddEventDoc() {
        try {
            ...
            $tASOReq = "";
            $tMethodReq = "GET";
            $aCompData = $this->Company_model->FSaMCMPList($tASOReq, $tMethodReq, $aDataWhereComp);
            ...
            $aCalInHDDisTemp = $this->Saleorder_model->FSaMSOCalInHDDisTemp($aCalDTTempParams);
            ...
            $aDataCustomer = array(
              ...
              $nDocType = $this->Saleorder_model->FSnMSOGetDocType();
            ...
            $this->Saleorder_model->FSxMSOAddUpdateHD($aDataMaster, $aDataWhere, $aTableAddUpdate);

            // Add Update HD Cst
            $this->Saleorder_model->FSxMSOAddUpdateHDCst($aDataCustomer,$aDataWhere, $aTableAddUpdate);

            // Update Doc No Into Doc Temp
            $this->Saleorder_model->FSxMSOAddUpdateDocNoToTemp($aDataWhere, $aTableAddUpdate);

            // Move Doc HD Dis Temp To HDDis
            $this->Saleorder_model->FSaMSOMoveHdDisTempToHdDis($aDataWhere, $aTableAddUpdate);

            // Move Doc DTTemp To DT
            $this->Saleorder_model->FSaMSOMoveDtTmpToDt($aDataWhere, $aTableAddUpdate);

            // Move Doc DTDisTemp To DTTemp
            $this->Saleorder_model->FSaMSOMoveDtDisTempToDtDis($aDataWhere, $aTableAddUpdate);
    ...
    public function FSvCSOCancelDocument() {
        try {
            ...
            $aStaApv = $this->Saleorder_model->FSaMSOCancelDocument($aDataUpdate);
            $aReturnData = $aStaApv;
        } catch (Exception $Error) {
            $aReturnData = array(
                'nStaEvent' => '500',
    ...
    //    $this->db->trans_begin();
        try {
            ...
            if($tCheckAprLeve['tReturnCode']=='200'){
              ...
              if($nTotalDocApr==$LastAprSeq){//หากอนุมัติครบจำนวนแล้วให้อัพเดทตาราง HD
                ...
                $aStaApv = $this->Saleorder_model->FSaMSOApproveDocument($aDataUpdate);
                $aReturn = array(
                    'nStaEvent' => '1',
        ...
        if(!empty($tSOtiemNotApr)){ //เฉพาะการอนุมัติรูปภาพ เพิ่มสินค้าที่ไม่ได้อนุมัติในตาราง DelOject
            ...
            $this->Saleorder_model->FSxMSONotAproveItem($aDataDelObj);
        }
    ...
    public function FSoCSOSearchAndAddPdtIntoTbl() {
        try {
            ...
            $aCountDataChkInDTTemp = $this->Saleorder_model->FSaCSOCountPdtBarInTablePdtBar($aDataChkINDB);
