...
class Adjuststocksub_controller extends MX_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('document/adjuststocksub/Adjuststocksub_model');
    }
    ...
    public function FSaCAdjStkSubAddTmpToDT($ptAjhDocNo = ''){
        ...
        $aResInsDT = $this->Adjuststocksub_model->FSaMAdjStkSubInsertTmpToDT($aDataWhere);
        if($aResInsDT['rtCode'] == '1'){
            $this->Adjuststocksub_model->FSxMAdjStkSubClearDTTmp($aDataWhere);
        }
    ...
    public function FSaCASTDeleteEvent(){
        ...
        $aResDel    = $this->Adjuststocksub_model->FSnMAdjStkSubDel($aDataMaster);
        $aReturn    = array(
    ...
    public function FSvCAdjStkSubRemovePdtInDTTmp(){
        ...
        $aResDel = $this->Adjuststocksub_model->FSnMAdjStkSubDelDTTmp($aDataWhere);
    ...
    public function FSxCAdjStkSubAddPage(){
        ...
        $this->Adjuststocksub_model->FSxMAdjStkSubClearDTTmp($aDataClearTmp);
        $aLocationList = $this->Adjuststocksub_model->FSaMAdjStkSubGetLocation($aDataClearTmp);
    ...
    public function FSaCAdjStkSubAddEvent(){
        try{
            ...
            $nCheckDateTime = $this->Adjuststocksub_model->FSnMASTCheckDateTimeTmpDT();
            if( $nCheckDateTime > 0 ){
                $aReturn = array(
                    'nStaEvent'    => '905',
            ...
            $this->Adjuststocksub_model->FSaMAdjStkSubAddUpdateHD($aDataMaster);
            $this->Adjuststocksub_model->FSaMAdjStkSubAddUpdateDocNoInDocTemp($aDataWhere); // Update DocNo ในตาราง Doctemp
    ...
    public function FSvCAdjStkSubEditPage(){
        ...
        // Get All Location
        $aLocationList = $this->Adjuststocksub_model->FSaMAdjStkSubGetLocation($aDataSearch);
        
        // Get Data
        $aAdjStkSubHD = $this->Adjuststocksub_model->FSaMAdjStkSubGetHD($aDataSearch); // GET Data TCNTPdtAdjStkHD
        $this->Adjuststocksub_model->FSaMAdjStkSubInsertDTToTemp($aDataSearch); // Insert Data DocTemp
    ...
    public function FSxCAdjStkSubDataTable(){
        ...
        $aResList   = $this->Adjuststocksub_model->FSaMAdjStkSubList($aDataSearch);
        $aGenTable  = array(
            'aAlwEvent'     => $aAlwEvent,
            'aDataList'     => $aResList,
            'nPage'         => $nPage,
    ...
    public function FSvCAdjStkSubPdtAdvTblLoadData(){
        ...
        $aDataDT = $this->Adjuststocksub_model->FSaMAdjStkSubGetDTTempListPage($aDataWhere);
    ...
    public function FSvCAdjStkSubPdtMultiDeleteEvent(){
        ...
        if($nCount > 1){
            ...
            for($i=0; $i<$nCount; $i++){
                ...
                $aResDel = $this->Adjuststocksub_model->FSaMAdjStkSubPdtTmpMultiDel($aDataMaster);
            }
    ...
    public function FSvCAdjStkSubCancel(){
        ...
        $aStaApv = $this->Adjuststocksub_model->FSvMAdjStkSubCancel($aDataUpdate); 
        if($aStaApv['rtCode'] == 1){
            $aApv = array(
                'nSta' => 1,
                'tMsg' => "Cancel done.",
    ...
    public function FSaCASTEventAddProducts(){
        try{
            ...
            $this->Adjuststocksub_model->FSxMAdjStkSubClearDTTmp($aDataClear);
            ...
            if( isset($aDataLocation) && !empty($aDataLocation) && count($aDataLocation) > 0 && empty($aDataCondition['oetASTFilterPlcCode'])){
                for($i=0;count($aDataLocation) > $i;$i++){
                    ...
                    $aResultPDT = $this->Adjuststocksub_model->FSaMASTEventAddProducts($aDataCondition,$aDataInsert);
                }
            }else{
                $aDataInsert = array(
                    'FTBchCode'     => $aGetDataInsert['tBchCode'],
    ...
    public function FSxCASTEditInLine(){
        ...
        $aEditInLine = $this->Adjuststocksub_model->FSaMASTEditInLine($aDataQuery);
    ...
    public function FSaCASTApprove(){
        ...
        $aStaApv = $this->Adjuststocksub_model->FSaMAdjStkSubApprove($aDataUpdate); 
    ...
    public function FSaCASTUpdateDateTime(){
        $aStaApv = $this->Adjuststocksub_model->FSaMASTUpdateDateTime(); 
        echo json_encode($aStaApv);
    }
