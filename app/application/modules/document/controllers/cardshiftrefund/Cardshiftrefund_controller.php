...
class Cardshiftrefund_controller extends MX_Controller {
    
    public function __construct(){
        parent::__construct();
        $this->load->model('document/cardshiftrefund/Cardshiftrefund_model');
    ...
    public function FSvCardShiftRefundDataList(){
        ...
        $tAPIReq = "";
        $tMethodReq = "GET";
        $aResList = $this->Cardshiftrefund_model->FSaMCardShiftRefundList($tAPIReq, $tMethodReq, $aData);
        $aAlwEvent = FCNaHCheckAlwFunc('cardShiftRefund/0/0');
        $aGenTable = array(
            'aAlwEvent' => $aAlwEvent,
            'aDataList' => $aResList,
    ...
    public function FSvCardShiftRefundDataSourceList(){
        ...
        if($tIsGetCardCodeMode == "1"){ // Use get card code mode
            ...
            if(!empty($tOptionDocNo)){ // Have document number
                // Get card in TopUpDT
                $aResList = $this->Cardshiftrefund_model->FSaMCardShiftRefundGetDTByDocNo($aData);
            }else{ // No have document number
                // Get card in card master
                $aResList = $this->Cardshiftrefund_model->FSaMCardShiftRefundDataSourceList($aData);
                if($tSetEmpty == "1"){
                    $aResList["raItems"] = [];
                }
            }
    ...
    public function FSvCardShiftRefundDataSourceListByFile(){
        ...
        $aResList = $this->Cardshiftrefund_model->FSaMCardShiftRefundDataSourceList($aData);
        $aCardShiftRefundHD = $this->Cardshiftrefund_model->FSaMCardShiftRefundSearchByID("", "", $aData);
    ...
    public function FSvCardShiftRefundEditPage(){
        ...
        $tAPIReq = "";
        $tMethodReq = "GET";
        $aCardShiftRefundData = $this->Cardshiftrefund_model->FSaMCardShiftRefundSearchByID($tAPIReq, $tMethodReq, $aData);
        ...
        $aCardShiftRefundCardDataInDT = $this->Cardshiftrefund_model->FSaMCardShiftRefundGetDTByDocNo($aData);
    ...
    public function FSaCardShiftRefundAddEvent(){
        try{
            ...
            $oCountDup = $this->Cardshiftrefund_model->FSoMCardShiftRefundCheckDuplicate($aDataMaster['FTCthDocNo']);
            $nStaDup = $oCountDup[0]->counts;
            
            if($nStaDup == 0){
                $this->db->trans_begin();
                
                $aStaCardShiftRefundHD = $this->Cardshiftrefund_model->FSaMCardShiftRefundAddUpdateHD($aDataMaster);
    ...
    public function FSaCardShiftRefundEditEvent(){
        try{
            ...
            $aCardShiftRefundHD = $this->Cardshiftrefund_model->FSaMCardShiftRefundSearchByID("", "", $aDataMaster);
            if( 
            ...
            ) 
            {
                ...
                if(count($aDataMaster['aCardCode']) <= 0){ // No have card
                    // 2.Remove old DT all
                    $this->Cardshifttopup_model->FSnMCardShiftRefundDelDT($aDataMaster);
                }
    ...
    public function FSaCardShiftRefundUpdateApvDocAndCancelDocEvent(){
        try{
            ...
            {   
                $aStaCardShiftRefundHD = $this->Cardshiftrefund_model->FSaMCardShiftRefundUpdateApvDocAndCancelDocHD($aDataMaster);
                /*========================== Approved =========================*/
                try{
                    $aMQParams = [
    ...
    public function FSaCardShiftRefundDeleteEvent(){
        ...
        $aResDel = $this->Cardshiftrefund_model->FSnMCardShiftRefundDel($aDataMaster);
        $aReturn = array(
    ...
    public function FStCardShiftRefundUniqueValidate($tSelect = ''){
        ...
        if($this->input->is_ajax_request()){ // Request check
            if($tSelect == 'cardShiftRefundCode'){
                ...
                $oCustomerGroup = $this->Cardshiftrefund_model->FSoMCardShiftRefundCheckDuplicate($tCardShiftRefundCode);
    ...
    public function FSoCardShiftRefundDeleteMulti(){
        ...
        foreach($aCardShiftRefundCode as $oCardShiftRefundCode){
            ...
            $this->Cardshiftrefund_model->FSnMCardShiftRefundDel($aCardShiftRefund);
        }
