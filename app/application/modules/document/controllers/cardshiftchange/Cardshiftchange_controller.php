...
class Cardshiftchange_controller extends MX_Controller {
    
    public function __construct(){
        parent::__construct();
        $this->load->model('document/cardshiftchange/Cardshiftchange_model');
    ...
    public function FSvCardShiftChangeDataList(){
        ...
        $tAPIReq = "";
        $tMethodReq = "GET";
        $aResList = $this->Cardshiftchange_model->FSaMCardShiftChangeList($tAPIReq, $tMethodReq, $aData);
        $aAlwEvent = FCNaHCheckAlwFunc('cardShiftChange/0/0');
        $aGenTable = array(
            'aAlwEvent' => $aAlwEvent,
            'aDataList' => $aResList,
    ...
    public function FSvCardShiftChangeDataSourceList(){
        ...
        if($tIsGetCardCodeMode == "1"){ // Use get card code mode
            ...
            if(!empty($tOptionDocNo)){ // Have document number
                // Get card in TopUpDT
                $aResList = $this->Cardshiftchange_model->FSaMCardShiftChangeGetDTByDocNo($aData);
            }else{ // No have document number
                // Get card in card master
                $aResList = $this->Cardshiftchange_model->FSaMCardShiftChangeDataSourceList($aData);
                if($tSetEmpty == "1"){
                    $aResList["raItems"] = [];
                }
            }
    ...
    public function FSvCardShiftChangeEditPage(){
        ...
        $tAPIReq = "";
        $tMethodReq = "GET";
        $aCardShiftChangeData = $this->Cardshiftchange_model->FSaMCardShiftChangeSearchByID($tAPIReq, $tMethodReq, $aData);
    ...
    public function FSaCardShiftChangeAddEvent(){
        try{
            ...
            $oCountDup  = $this->Cardshiftchange_model->FSoMCardShiftChangeCheckDuplicate($aDataMaster['FTCvhDocNo']);
            $nStaDup    = $oCountDup[0]->counts;
            
            if($nStaDup == 0){
                $this->db->trans_begin();
                
                $aStaCardShiftChangeHD = $this->Cardshiftchange_model->FSaMCardShiftChangeAddUpdateHD($aDataMaster);
    ...
    public function FSaCardShiftChangeEditEvent(){
        try{
            ...
            $aCardShiftChangeHD = $this->Cardshiftchange_model->FSaMCardShiftChangeSearchByID("", "", $aDataMaster);
    ...
    public function FSaCardShiftChangeUpdateApvDocAndCancelDocEvent(){
        try{
            ...
            {
                $aStaCardShiftChangeHD = $this->Cardshiftchange_model->FSaMCardShiftChangeUpdateApvDocAndCancelDocHD($aDataMaster);
    ...
    public function FSaCardShiftChangeDeleteEvent(){
        ...
        $aResDel = $this->Cardshiftchange_model->FSnMCardShiftChangeDel($aDataMaster);
        $aReturn = array(
    ...
    public function FStCardShiftChangeUniqueValidate($tSelect = ''){
        ...
        if($this->input->is_ajax_request()){ // Request check
            if($tSelect == 'cardShiftChangeCode'){
                ...
                $oCardShiftChangeHD = $this->Cardshiftchange_model->FSoMCardShiftChangeCheckDuplicate($tCardShiftChangeCode);
    ...
    public function FStCardShiftChangeCardUniqueValidate($tSelect = ''){
        ...
        if($this->input->is_ajax_request()){ // Request check
            ...
            if($tSelect == 'cardShiftChangeCardCode'){
                $oCardShiftChangeCardCode = $this->Cardshiftchange_model->FSoMCardShiftChangeCheckCardDuplicate($tCardShiftChangeCardCode);
            ...
            if($tSelect == 'cardShiftChangeCardCodeInDT'){
                $oCardShiftChangeCardCodeInDT = $this->Cardshiftchange_model->FSoMCardShiftChangeCheckCardDuplicateInDT($tCardShiftChangeCardCode);
    ...
    public function FSoCardShiftChangeDeleteMulti(){
        ...
        foreach($aCardShiftChangeCode as $oCardShiftChangeCode){
            ...
            $this->Cardshiftchange_model->FSnMCardShiftChangeDel($aCardShiftChange);
        }
