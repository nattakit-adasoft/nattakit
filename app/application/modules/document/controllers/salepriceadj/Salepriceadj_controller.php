...
class Salepriceadj_controller extends MX_Controller {
    public function __construct(){
        parent::__construct ();
        $this->load->model('company/company/Company_model');
        $this->load->model('document/salepriceadj/Salepriceadj_model');
    ...
    public function FSvCSPADataList(){ 
        try{
            ...
            $aSpaDataList               = $this->Salepriceadj_model->FSaMSPAList($aData); 
            $aAlwEventSalePriceAdj	    = FCNaHCheckAlwFunc('dcmSPA/0/0');
            $aGenTable  = array(
    ...
    public function FSvCSPAPdtPriDataList(){ 
        try{
            ...
            $aColumnShow                    = $this->Salepriceadj_model->FCNaDCLGetColumnShow('TCNTPdtAdjPriDT');
            $aPdtPriDataList                = $this->Salepriceadj_model->FSaMSPAPdtPriList($aData);
            $aAlwEventSalePriceAdj	        = FCNaHCheckAlwFunc('dcmSPA/0/0');
            $aGenTable  = array(
    ...
    public function FSvCSPAAddPage(){
        ...
        // ================== Create By Witsarut 27/08/2019 ===================
            ...
            $aDelTmp   = $this->Salepriceadj_model->FSaMSPADelPdtTmp($aData);
    ...
    public function FSvCSPAEditPage(){
        try{
            ...
            // $aDTAddTmp = $this->Salepriceadj_model->FSaMSPADTList($aDataDel);
            // $oTmptoDT  = $this->Salepriceadj_model->FSoMSPATmptoDT($aDataDoc); 
            $oDelTmp   = $this->Salepriceadj_model->FSaMSPADelPdtTmp($aDataDoc);
            $oDTtoTmp  = $this->Salepriceadj_model->FSoMSPADTtoTmp($aDataDoc);
            ...
            $aSpaData    = $this->Salepriceadj_model->FSaMSPAGetDataByID($aData);
    ...
    public function FSoCSPAAddEvent(){
        ...
        if($tXphDocNo != ''){
            try{
                ...
                $oCountDup = $this->Salepriceadj_model->FSnMSPACheckDuplicate($aDataSpa['FTXphDocNo']);
                ...
                if($oCountDup !== FALSE && $oCountDup['counts'] == 0){
                    ...
                    //     'FTXphTStart' => $this->input->post('oetXphTStart')
                    // );
                    // $aCountDupPdtPriHD = $this->Salepriceadj_model->FSnMSPACheckDuplicatePdtPriHD($aDataChkDupDateTime);

                    // if($aCountDupPdtPriHD !== FALSE && $aCountDupPdtPriHD['counts'] == 0){

                        $this->db->trans_begin();
                        
                        $this->Salepriceadj_model->FSaMSPAAddUpdateDocNoInDocTemp($aDataSpa); // Update Docno in DocTemp
                        $this->Salepriceadj_model->FSaMSPAAddUpdateMaster($aDataSpa); // Update to HD

                        // Add Product into DocTmpDT
                        if($this->input->post('nStaAction') == '1'){
                            $this->Salepriceadj_model->FSaMSPADelAllProductDT($aDataSpa); // Delete All Product by id from table DT
                            $this->Salepriceadj_model->FSoMSPATmptoDT($aDataSpa); // Move Doc temp to DT
                        }
    ...
    public function FSoCSPAEditEvent(){  
        try{
            ...
            // $aCountDupPdtPriHD = $this->Salepriceadj_model->FSnMSPACheckDuplicatePdtPriHD($aDataSpa);

            // if($aCountDupPdtPriHD !== FALSE && $aCountDupPdtPriHD['counts'] == 0){

                $this->Salepriceadj_model->FSaMSPAAddUpdateMaster($aDataSpa);
                $this->Salepriceadj_model->FSaMSPADelAllProductDT($aDataSpa);
                $this->Salepriceadj_model->FSoMSPATmptoDT($aDataSpa); // Move Doc temp to DT
    ...
    public function FSoCSPADeleteEvent(){ 
        ...
        $aResDel = $this->Salepriceadj_model->FSaMSPADelAll($aDataMaster);
        $aReturn = array(
    ...
    public function FSoCSPAPdtPriDeleteEvent(){
        ...
        if($tDelType == "M"){ // Delete Multiple
            foreach($aPdtDataItem as $aPdtData){
                ...
                $aResDel = $this->Salepriceadj_model->FSaMSPAPdtTmpDelAll($aDataMaster);
        ...
        // $oDelAllPdt     = $this->Salepriceadj_model->FSaMSPADelAllProductDT($aDataDocTemp);            //Remove All Product from dt
        // $oTmptoDT       = $this->Salepriceadj_model->FSoMSPATmptoDT($aDataDocTemp);                    //Move Doc temp to DT
        // $oDelAllTmp     = $this->Salepriceadj_model->FSaMSPADelPdtTmp($aDataDocTemp);
        // $oDTtoTmp       = $this->Salepriceadj_model->FSoMSPADTtoTmp($aDataDocTemp);                    //Move DT to Doc temp
    ...
    public function FSoCSPAGetBchComp(){
        $aGetBch = $this->Login_model->FSaMLOGGetBch();
    ...
    public function FSvCSPAPdtPriAddTmpEvent(){
        try{
            ...
            $aGetSeq = $this->Salepriceadj_model->FSaMSPACheckDataSeq($aDataGetSeq);
            $nSeq    = $aGetSeq[0]['nSeq'];
            ...
            if($aGetSeq !== FALSE){
                for($i=0;$i<$nNumData;$i++){
                    ...
                    $aCheckTmpDup = $this->Salepriceadj_model->FSaMSPACheckDataTempDuplicate($aDataEdit); //check data duplicate

                    // insert data to table doctmp if not have items
                    if($aCheckTmpDup == FALSE){
                        $this->Salepriceadj_model->FSaMSPAAddPdtDocTmp($aDataEdit);
                    }
                }
    ...
    public function FSvCSPAPdtPriAddDTEvent(){
        try{
            ...
            $this->Salepriceadj_model->FSaMSPADelAllProductDT($aData);

            if($this->input->post('nStaAction') == '1'){

                $oDTtoTmp = $this->Salepriceadj_model->FSoMSPADTtoTmp($aData);
                $oTmptoDT = $this->Salepriceadj_model->FSoMSPATmptoDT($aData);
    ...
    public function FSoCSPAUpdatePriceTemp(){
        ...
        $nOptDecimalShow = FCNxHGetOptionDecimalShow();
        $aResDel = $this->Salepriceadj_model->FSaMSPAUpdatePriceTemp($aDataMaster);
        $aResultPreice = $this->Salepriceadj_model->FSoMSPAGetPdtPriPageAll($aDataMaster['tSearchSpaPdtPri'],$aDataMaster['FTXthDocNo'],$aDataMaster['FTSessionID'],1,'TCNTPdtAdjPriHD');
        $aReturn = array(
            'nStaEvent' => $aResDel['rtCode'],
            'tStaMessg' => $aResDel['rtDesc'],
    ...
    public function FSoCSPAOriginalPrice(){
        ...
        $aPdtData4PDT = $this->Salepriceadj_model->FSaMSPADataPrice4Pdt($aWherePriceList);
    ...
    public function FSoCSPAPdtPriAdjustEvent(){
        ...
        $aTempData  = $this->Salepriceadj_model->FSaMSPAGetDataFromTemp($aDataMaster);
        ...
        for($i=0;$i<$nCountTempData;$i++){
            ...
            $aEditTempData  = $this->Salepriceadj_model->FSaMSPAUpdatePriceTemp($aDataChangePrice);
    ...
    public function FSoCSPAApproveEvent(){
        try{
            ...
            // if($aCountDupPdtPriHD !== FALSE && $aCountDupPdtPriHD['counts'] == 0){
                ...
                $this->Salepriceadj_model->FSaMSPAApprove($aDataUpdate);
    ...
    public function FSoCSPAUpdateStaDocCancel(){
        ...
        $aStaDoc = $this->Salepriceadj_model->FSaMSPAUpdateStaDocCancel($aDataUpdate); 
        $aReturn    = array(
