...
class Producttransferwahousevd_controller extends MX_Controller {
	...
	public function __construct() {
            ...
            $this->load->model('document/producttransferwahousevd/Producttransferwahousevd_model');
    }
    ...
    public function FSaCTFWEditEvent(){
        try{
            ...
            $aStaEventOrdHDRef          = $this->Producttransferwahousevd_model->FSaMTFWAddUpdateHDRef($aDataHDSpl);
    ...
    */
    ...
    public function FSvCTFWEditPage(){
       ...
       //Lang ภาษา
        ...
        //Get Data
        $aResult        = $this->Producttransferwahousevd_model->FSaMTFWGetHD($aDataWhere);       // Data TVDTPdtTwxHD
        $aDataHDRef     = $this->Producttransferwahousevd_model->FSaMTFWGetHDRef($aDataWhere);    // Data TVDTPdtTwxHDRef
        // $aDataDT        = $this->Producttransferwahousevd_model->FSaMTFWGetDT($aDataWhere);       // Data TVDTPdtTwxDT
        // // $aStaIns        = $this->Producttransferwahousevd_model->FSaMTFWInsertDTToTemp($aDataDT,$aDataWhere);    // Insert Data DocTemp
    ...
    public function FSxCTWXVDPdtDtLoadToTem(){
        ...
        if (!($tDocno != '' && ($tStaApv == 1 && $tStaPrcStk == 1))) {
            ...
            $aDataInfor = $this->Producttransferwahousevd_model->FSaMTWXVDGetPdtLayoutToTemFromDT($aInfoWhere);
        ...
        if($aData){
            if($tPanelPdtType==1){ // pos to shop
                ...
                if (!($tDocno != '' && ($tStaApv == 1 && $tStaPrcStk == 1))) {
                    ...
                    for($nI=0;$nI<count($aData);$nI++){
                        ...
                        $nStkPdtStart = $this->Producttransferwahousevd_model->FSnMGetStkPdt($aInfoWhere);
                    ...
                    for($nI=0;$nI<count($aShopKey);$nI++){
                        if($aShopKey[$nI]["pdt"]!="NULL"){
                            for($nJ=0;$nJ<count($aShopKey);$nJ++){
                ...
                if (!($tDocno != '' && ($tStaApv == 1 && $tStaPrcStk == 1))) {
                    for($nI=0;$nI<count($aData);$nI++){
                        ...
                        $nRealTransferStartInfor = $this->Producttransferwahousevd_model->FSaMTWXVDGetRealTransferPosStart($aInfoWhere);
            ...
            if (!($tDocno != '' && ($tStaApv == 1 && $tStaPrcStk == 1))) {
                if($this->input->post("nStatusSetDTForEdit")==1){
                    ...
                    if($tRoute!="TWXVDEventAdd"){
                        $aDataUserInput = $this->Producttransferwahousevd_model->FSxMGetDataFromDtToTmp(array(
                            "tBchCode"=>$this->input->post("tBchCode"),
                            "tXthDocNo"=>$this->input->post("tXthDocNo")
                        ));
            ...
            ////////////////////////////////////////
            //if($nCheckInsertPdt!=count($aData)){

            
                for($nI=0;$nI<count($aData);$nI++){
                    $tFCUserInPutTransferForTWXVD = $aData[$nI]["maxTransfer"];
                    if($aDataUserInput){
                        for($nJ=0;$nJ<count($aDataUserInput);$nJ++){
