...
class cPromotion extends MX_Controller {
    ...
    public function FSxCPMTAddPage(){
        ...
        $aSpmData =  $this->mPromotion->FSaMPMTTSysPmtSearchByID($aData);
    ...
    public function FSvCPMTEditPage(){
        ...
        $aResult        = $this->mPromotion->FSaMPMTGetPdtPmtHD($aDataEdit); // Data TCNTPdtPmtHD Promotion
        $aResultPmtCD   = $this->mPromotion->FSaMPMTPdtPmtCD($aDataEdit); // Data TCNTPdtPmtCD Condition
        $aResultPmtDT   = $this->mPromotion->FSaMPMTPdtPmtDT($aDataEdit); // Data TCNTPdtPmtDT Detail
        $aResultPmtGrpBoth = $this->mPromotion->FSaMPMTPdtPmtDTGrpBoth($aDataEdit); // Data TCNTPdtPmtDT GrpBoth Only
        ...
        $aSpmData =  $this->mPromotion->FSaMPMTTSysPmtSearchByID($aDataTSysPmt);
    ...
    public function FSaCPMTAddEvent(){
        ...
        try{
                ...
                $oCountDup  = $this->mPromotion->FSnMPMTCheckDuplicate($aDataMaster['FTPmhCode']);
                ...
                if($nStaDup == 0){
                    ...
                    //Master Pmt
                    $aStaEventMaster  = $this->mPromotion->FSaMPMTAddUpdatePmtHD($aDataMaster);
                    //Master Pmt
    ...
    public function FSaCPMTEditEvent(){
        ...
        try{
                ...
                $this->db->trans_begin();
                    $aStaDstMaster  = $this->mPromotion->FSaMPMTAddUpdatePmtHD($aDataMaster);
    ...
    public function FSaMPMTAddPmtDT($paCondition,$paGrpBuy,$paGrpJoin,$paGrpRcv,$paGrpBothItem,$paGrpReject){
        ...
        $aStaEventDelCD =  $this->mPromotion->FSnMPMTDelPmtCD($tPmhCode); /*ลบ Data เก่าออก*/
        $aStaEventDelDT =  $this->mPromotion->FSnMPMTDelPmtDT($tPmhCode); //*ลบ Data เก่าออก*/
         ...
         for($i=0;$i<$nNumCondition;$i++){
             ...
             $this->db->trans_begin();
             $aStaEventCD = $this->mPromotion->FSaMPMTAddUpdatePmtCD($aDataCondition);
             if($this->db->trans_status() === false){
                 $this->db->trans_rollback();
             }else{
                 $this->db->trans_commit();
        ...
        for($i=0;$i<$nNumGrpBuy;$i++){
            ...
            $this->db->trans_begin();
            $aStaEventCD = $this->mPromotion->FSaMPMTAddUpdatePmtDT($aDataCondition);
            if($this->db->trans_status() === false){
                $this->db->trans_rollback();
            }else{
                $this->db->trans_commit();
        ...
        for($nItem=0;$nItem<$nNumBothItem;$nItem++){
            ...
            $nNumGrpBoth = count($aGrpBoth);
                ...
                for($i=0;$i<$nNumGrpBoth;$i++){
                    ...
                    $aStaEventCD = $this->mPromotion->FSaMPMTAddUpdatePmtDT($aDataGrpBoth);
    ...
    public function FSaCPMTDeleteEvent(){
        ...
        $aResDel    = $this->mPromotion->FSnMPMTDel($aDataMaster);
        $aReturn    = array(
    ...
    public function FSxCPMTDataTable(){
        ...
        $aResList   = $this->mPromotion->FSaMPMTList($aData);
        $aGenTable  = array(
    ...
    public function FSxCPMTTSysListDataTable(){
        ...
        $aResList   = $this->mPromotion->FSaMPMTTSysPmtList($aData);
        $aGenTable  = array(
