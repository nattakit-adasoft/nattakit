...
class Cardshiftout_model extends CI_Model {
    ...
    public function FSaMCardShiftOutSearchByID($ptAPIReq, $ptMethodReq, $paData){
        $tCardShiftOutDocNo = $paData['FTCshDocNo'];
        $nLngID = $paData['FNLngID'];
        $tSQL = "SELECT
                    CRDHD.FTCshDocNo AS rtCardShiftOutDocNo,
                    CRDHD.FTBchCode AS rtCardShiftOutBchCode,
                    CRDHD.FTCshDocType AS rtCardShiftOutDocType,
                    CRDHD.FDCshDocDate AS rtCardShiftOutDocDate,
                    CRDHD.FTUsrCode AS rtCardShiftOutUsrCode,
                    CRDHD.FTCshApvCode AS rtCardShiftOutApvCode,
                    CRDHD.FDCshApvDate AS rtCardShiftOutApvDate,
                    CRDHD.FTCshCashier AS rtCardShiftOutCashier,
                    CRDHD.FTCshStaSetExpired AS rtCardShiftOutStaSetExpired,
                    CRDHD.FDCshDateExpired AS rtCardShiftOutDateExpired,
                    CRDHD.FNCshCardQty AS rtCardShiftOutCardQty,
                    CRDHD.FTCshStaPrcDoc AS rtCardShiftOutStaPrcDoc,
                    CRDHD.FTCshStaDoc AS rtCardShiftOutStaDoc,
                    CRDHD.FTCshStaDelMQ AS rtCardShiftOutStaDelMQ
                FROM [TFNTCrdShiftHD] CRDHD
                WHERE 1=1";
                $tSQL .= " AND CRDHD.FTCshDocNo = '$tCardShiftOutDocNo'";
                $tSQL .= " AND CRDHD.FTCshDocType = '1'";
        
        /*if($tCardShiftOutDocNo != ""){
            $tSQL .= "AND CRDHD.FTCshDocNo = '$tCardShiftOutDocNo'";
        }*/
    ...
    public function FSaMCardShiftOutList($ptAPIReq, $ptMethodReq, $paData){
        ...
        $tSQL = "SELECT c.* FROM(
                    SELECT ROW_NUMBER() OVER(ORDER BY rtCardShiftOutDocDate DESC) AS rtRowID,*
                    FROM
                        (SELECT DISTINCT
                            CRDHD.FTBchCode AS rtCardShiftOutBchCode,
                            CRDHD.FTCshDocNo AS rtCardShiftOutDocNo,
                            CRDHD.FDCshDocDate AS rtCardShiftOutDocDate,
                            CRDHD.FNCshCardQty AS rtCardShiftOutCshCardQty,
                            CRDHD.FTCshStaDoc AS rtCardShiftOutCshStaDoc,
                            CRDHD.FTCshStaPrcDoc AS rtCardShiftOutCshStaPrcDoc
                        FROM [TFNTCrdShiftHD] CRDHD
                        WHERE 1=1";
                        
                        // BchCode is empty = HQ(use all)
                        ($tUserLevel == "HQ") ? "" : $tSQL .= " AND CRDHD.FTBchCode = '$tCardShiftOutBchCode'";                          
                        $tSQL .= " AND CRDHD.FTCshDocType = '1'";    
    ...
    public function FSnMCardShiftOutGetPageAll($ptSearchList, $paData){
        ...
        $tSQL = "SELECT COUNT (CRDHD.FTBchCode) AS counts
                ...
                // BchCode is empty = HQ(use all)
                empty($tCardShiftOutBchCode) ? "" : $tSQL .= " AND CRDHD.FTBchCode = '$tCardShiftOutBchCode'"; 
                $tSQL .= " AND CRDHD.FTCshDocType = '1'";
    ...
    public function FSoMCardShiftOutCheckDuplicate($ptCardShiftOutCode){
        $tSQL = "SELECT COUNT(FTCshDocNo) AS counts
                    FROM TFNTCrdShiftHD
                    WHERE FTCshDocNo = '$ptCardShiftOutCode'";
        $oQuery = $this->db->query($tSQL);
        if($oQuery->num_rows() > 0){
            return $oQuery->result();
        }else{
    ...
    public function FSaMCardShiftOutAddUpdateDTBySeq($paData){
        try{
            // Update Card
            $this->db->set('FTCrdStaPrc' , $paData['FTCrdStaPrc']);
            $this->db->set('FDLastUpdOn' , $paData['FDLastUpdOn']);
            $this->db->set('FTLastUpdBy' , $paData['FTLastUpdBy']);
            $this->db->where('FNCsdSeqNo' , $paData['FNCsdSeqNo']);
     ...
     * Return Type : array
     */
    public function FSnMCardShiftOutDel($paData){
        $this->db->where('FTCshDocNo', $paData['FTCshDocNo']);
    ...
    public function FSaMCardShiftOutGetDTByDocNo($paData){
        ...
        $nLngID     = $paData['FNLngID'];
        $tSQL = "SELECT
                    CRDDT.FTCshDocNo AS rtCardShiftOutDocNo,
                    CRDDT.FTBchCode AS rtCardShiftOutBchCode,
                    CRDDT.FTCshDocNo AS rtCardShiftOutDocNo,
                    CRDDT.FTCrdCode AS rtCardShiftOutCrdCode,
                    CRDDT.FCCtdCardBal AS rtCardShiftOutCardBal,
                    CRDDT.FNCsdSeqNo AS rtCardShiftOutSeqNo,
                    CRDDT.FTCrdStaCrd AS rtCardShiftOutStaCrd,
                    CRDDT.FTCrdRmk AS rtCardShiftOutRmk,
                ...
                AND CRDDT.FTCshDocNo = '$tCardShiftOutDocNo'";
