...
class Couriergrp_controller extends MX_Controller {
    public function __construct(){
        parent::__construct ();
        $this->load->model('courier/couriergrp/Couriergrp_model');
    ...
    public function FSvCCGPDataList(){ 
        try{
            ...
            $aCpgDataList           = $this->Couriergrp_model->FSaMCpgList($aData); 
            $aAlwEventCourierGrp	    = FCNaHCheckAlwFunc('courierGrp/0/0');
            $aGenTable  = array(
    ...
    public function FSvCCGPEditPage(){  
        try{
            ...
            $aCpgData       = $this->Couriergrp_model->FSaMCpgGetDataByID($aData);
            $aDataCourierGrp   = array(
                'nStaAddOrEdit' => 1,
                'aCpgData'      => $aCpgData
            );
    ...
    public function FSoCCGPAddEvent(){
        try{
            ...
            $oCountDup      = $this->Couriergrp_model->FSnMCpgCheckDuplicate($aDataCourierGrp['FTCgpCode']);
            $nStaDup        = $oCountDup['counts'];
            if($oCountDup !== FALSE && $nStaDup == 0){
                $this->db->trans_begin();
                $aStaCpgMaster  = $this->Couriergrp_model->FSaMCpgAddUpdateMaster($aDataCourierGrp);
                $aStaCpgLang    = $this->Couriergrp_model->FSaMCpgAddUpdateLang($aDataCourierGrp);
    ...
    public function FSoCCGPDeleteEvent(){ 
        ...
        $aResDel    = $this->Couriergrp_model->FSaMCpgDelAll($aDataMaster);
        $nNumRowCpg = $this->Couriergrp_model->FSnMPGPGetAllNumRow();
        if($nNumRowCpg!==false){
            $aReturn    = array(
