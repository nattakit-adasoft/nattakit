...
class Couriertype_controller extends MX_Controller {
    public function __construct(){
        parent::__construct ();
        $this->load->model('courier/couriertype/Couriertype_model');
    ...
    public function FSvCCTYDataList(){
        try{
            ...
            $aCtyDataList           = $this->Couriertype_model->FSaMCTYList($aData); 
            $aAlwEventCourierType	= FCNaHCheckAlwFunc('courierType/0/0');
            $aGenTable   = array(
    ...
    function FSvCCTYEditPage(){
        try{
            ...
            $aCtyData   = $this->Couriertype_model->FSaMCTYGetDataByID($aData);
            $aDataCourierType   = array(
                'nStaAddOrEdit' => 1,
                'aCtyData'      => $aCtyData
            );
    ...
    public function FSoCCTYAddEvent(){
        try{
            ...
            $oCountDup      = $this->Couriertype_model->FSnMCTYCheckDuplicate($aDataCourierType['FTCtyCode']);
            $nStaDup        = $oCountDup['counts'];
            if($oCountDup !== FALSE && $nStaDup == 0){
                $this->db->trans_begin();
                $aStaCtyMaster  = $this->Couriertype_model->FSaMCTYAddUpdateMaster($aDataCourierType);
                $aStaCtyLang    = $this->Couriertype_model->FSaMCTYAddUpdateLang($aDataCourierType);
    ...
    public function FSoCCTYDeleteEvent(){
        ...
        $aResDel    = $this->Couriertype_model->FSaMCTYDelAll($aDataMaster);
        $nNumRowCpg = $this->Couriertype_model->FSnMCTYGetAllNumRow();
        if($nNumRowCpg!==false){
            $aReturn    = array(
