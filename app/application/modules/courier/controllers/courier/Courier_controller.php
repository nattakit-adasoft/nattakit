...
class Courier_controller extends MX_Controller {

	public function __construct() {
        parent::__construct ();
		$this->load->model('courier/courier/Courier_model');
    ...
    public function FSvCCRYDataList(){
		...
		$aGetDataList = $this->Courier_model->FSaMCRYDataList($aDatalist);
	...
	public function FSvCCRYEditPage(){
		...
		$aGetDataList = $this->Courier_model->FSaMCRYGetDataByID($aDataCourier);
		$aGetImage    = $this->Courier_model->FSaMCRYGetImageByID($aDataCourier);
    ...
    public function FSoCCRYAddEvent(){
		...
		if($tCryCode != ""){
			...
			$aCountDup = $this->Courier_model->FSaMCRYCheckDuplicate($aDataCourier['FTCryCode']);
			if($aCountDup !== FALSE && $aCountDup['counts'] == 0){

				$aStaCryMaster  = $this->Courier_model->FSaMCRYUpdateMaster($aDataCourier);
				$aStaCryLang  	= $this->Courier_model->FSaMCRYUpdateLang($aDataCourierLang);
    ...
    public function FSoCCRYDeleteEvent(){
        ...
        $aResDel    	= $this->Courier_model->FSaMCRYDelAll($aDataMaster);
