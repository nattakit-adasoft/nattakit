...
class Reason_controller extends MX_Controller {

    public function __construct() {
        parent::__construct ();
        $this->load->model('other/reason/Reason_model');
    ...
    public function FSvRSNDataList(){
        ...
        $tAPIReq    = "";
        $tMethodReq = "GET";
        $aResList   = $this->Reason_model->FSaMRSNList($tAPIReq,$tMethodReq,$aData);
    ...
    public function FSvRSNAddPage(){
        ...
        $tAPIReq        = "";
        $tMethodReq     = "GET";
        $aRsnSysData    = $this->Reason_model->FSaMRSNSysGroup($tAPIReq,$tMethodReq,$aData);
    ...
    public function FSvRSNEditPage(){
        ...
        $tAPIReq        = "";
        $tMethodReq     = "GET";
        $aRsnData       = $this->Reason_model->FSaMRSNSearchByID($tAPIReq,$tMethodReq,$aData);
        //Create Selecte Box --
        $aRsnSysData    = $this->Reason_model->FSaMRSNSysGroup($tAPIReq,$tMethodReq,$aData);
    ...
    public function FSaRSNAddEvent(){
        try{
            ...
            $oCountDup  = $this->Reason_model->FSoMRSNCheckDuplicate($aDataMaster['FTRsnCode']);
            $nStaDup    = $oCountDup[0]->counts;
            if($nStaDup == 0){
                $this->db->trans_begin();
                $aStaRsnMaster  = $this->Reason_model->FSaMRSNAddUpdateMaster($aDataMaster);
                $aStaRsnLang    = $this->Reason_model->FSaMRSNAddUpdateLang($aDataMaster);
    ...
    public function FSaRSNDeleteEvent(){
        ...
        $tMethodReq     = 'POST';
        $aResDel        = $this->Reason_model->FSnMRSNDel($tAPIReq,$tMethodReq,$aDataMaster);
        $nNumRowRsnLoc  = $this->Reason_model->FSnMLOCGetAllNumRow();
