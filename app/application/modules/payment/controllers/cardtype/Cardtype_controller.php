...
class Cardtype_controller extends MX_Controller {
    ...
    public function FSvCCTYDataList(){
        try{
            ...
            $aCtyDataList   = $this->Cardtype_model->FSaMCTYList($aData); 
    ...
    public function FSvCCTYEditPage(){
        try{
            ...
            $aCtyData       = $this->Cardtype_model->FSaMCTYGetDataByID($aData);
            $aDataCardType   = array(
                'nStaAddOrEdit' => 1,
                'aCtyData'      => $aCtyData
            );
    ...
    public function FSoCCTYAddEvent(){ 
        try{
            ...
            $oCountDup      = $this->Cardtype_model->FSnMCTYCheckDuplicate($aDataCardType['FTCtyCode']);
            $nStaDup        = $oCountDup['counts'];
            if($oCountDup !== FALSE && $nStaDup == 0){
                $this->db->trans_begin();
                $aStaCtyMaster  = $this->Cardtype_model->FSaMCTYAddUpdateMaster($aDataCardType);
                $aStaCtyLang    = $this->Cardtype_model->FSaMCTYAddUpdateLang($aDataCardType);
    ...
    public function FSoCCTYDeleteEvent(){
        try{
            ...
            $aResDel     = $this->Cardtype_model->FSaMCTYDelAll($aDataMaster);
            // $nNumRowCtyLoc = $this->Cardtype_model->FSnMLOCGetAllNumRow();
