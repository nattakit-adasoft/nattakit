...
class Creditcard_controller extends MX_Controller {

    public function __construct(){
        date_default_timezone_set("Asia/Bangkok");
        parent::__construct();
        $this->load->model('creditcard/creditcard/Creditcard_model');
    }
    ...
    public function FSaCCDCAddEvent(){
        try{
            ...
            $oCountDup  = $this->Creditcard_model->FSnMCRDCheckDuplicate($aDataMaster['FTCrdCode']);
            $nStaDup    = $oCountDup[0]->counts;
            if($nStaDup == 0){
                $this->db->trans_begin();
                $aStaEventMaster  = $this->Creditcard_model->FSaMCDCAddUpdateMaster($aDataMaster);
                $aStaEventLang    = $this->Creditcard_model->FSaMCDCAddUpdateLang($aDataMaster);
                if($this->db->trans_status() === false){
                    ...
                    if($tImgInputCrd != '' || $tImgInputCrdOld != ''){
                        ...
                        $this->Creditcard_model->FSaMCDCAddUpdateImgObj($aImageUplode);
                    }
                    $aReturn = array(
    ...
    public function FSvCCDCEditPage(){
        ...
        $aResult       = $this->Creditcard_model->FSaMCDCSearchByID($aData);
    ...
    public function FSaCCDCDeleteEvent(){
        ...
        $aResDel    = $this->Creditcard_model->FSnMCDCDel($aDataMaster);
        $aDeleteImage           = array(
                'tModuleName'  => 'creditcard',
                'tImgFolder'   => 'creditcard',
                'tImgRefID'    => $tIDCode ,
    ...
    public function FSxCCDCDataTable(){
        ...
        $aResList   = $this->Creditcard_model->FSaMCDCList($aData);
        $aGenTable  = array(
            'aAlwEvent'     => $aAlwEvent,
            'aDataList'     => $aResList,
            'nPage'         => $nPage,
