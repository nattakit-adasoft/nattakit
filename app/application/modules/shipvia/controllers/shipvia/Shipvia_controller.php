...
class Shipvia_controller extends MX_Controller {
    public function __construct(){
        parent::__construct ();
        $this->load->model('shipvia/shipvia/Shipvia_model');
    }
    ...
    public function FSvCVIADataList(){
        try{
            ...
            $aViaDataList   = $this->Shipvia_model->FSaMVIAList($aData); 
            $aGenTable  = array(
    ...
    public function FSvCVIAEditPage(){
        try{
            ...
            $aViaData       = $this->Shipvia_model->FSaMVIAGetDataByID($aData);
            $aDataShipVia   = array(
                'nStaAddOrEdit' => 1,
                'aViaData'      => $aViaData
            );
    ...
    public function FSoCVIAAddEvent(){ 
        try{
            ...
            $oCountDup      = $this->Shipvia_model->FSnMVIACheckDuplicate($aDataShipVia['FTViaCode']);
            $nStaDup        = $oCountDup['counts'];

            if($oCountDup !== FALSE && $nStaDup == 0){
                $this->db->trans_begin();
                $aStaViaMaster  = $this->Shipvia_model->FSaMVIAAddUpdateMaster($aDataShipVia);
                $aStaViaLang    = $this->Shipvia_model->FSaMVIAAddUpdateLang($aDataShipVia);
    ...
    public function FSoCVIADeleteEvent(){
        ...
        $aResDel    = $this->Shipvia_model->FSaMVIADelAll($aDataMaster);
        $aReturn    = array(
