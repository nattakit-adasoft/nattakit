...
class Posedc_controller extends MX_Controller {

    public function __construct() {
        parent::__construct ();
        $this->load->model('pos/posedc/Posedc_model');
    ...
    public function FSoCPosEdcDataTable(){
        try{
            ...
            $aDataPosEdcList    = $this->Posedc_model->FSaMGetDataPosEdcList($aDataWhere);
            $aAlwEvent          = FCNaHCheckAlwFunc('posEdc/0/0');
            $aConfigView    = array(
                'nPage'     => $nPage,
                'aAlwEvent' => $aAlwEvent,
    ...
    public function FSoCPosEdcCallPageEdit(){
        try{
            ...
            $aDataPosEdc    = $this->Posedc_model->FSaMPosEdcGetDataByID($aDataWhere);
    ...
    public function FSoCPosEdcAddEvent(){
        try{
            ...
            $this->Posedc_model->FSxMPosEdcAddUpdateMain($aDataMaster);
            $this->Posedc_model->FSxMPosEdcAddUpdateLang($aDataMaster);
    ...
    public function FSoCPosEdcDeleteEvent(){
        try{
            ...
            $aStaDelRole        = $this->Posedc_model->FSaMPosEdcDeleteData($aDataWhere);
            if($aStaDelRole['rtCode'] == '1'){
                $nNumRowPosEdc  = $this->Posedc_model->FSnMCountDataPosEdc();
                $aReturnData    = array(
