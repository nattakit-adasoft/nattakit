...
class Salemachinedevice_controller extends MX_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('pos/salemachinedevice/Salemachinedevice_model');
    }
    ...
    public function FSvCPHWDataList()
    {
        try {
            ...
            $aPhwDataList   = $this->Salemachinedevice_model->FSaMPHWList($aData);
            $aGenTable  = array(
    ...
    public function FSvCPHWAddPage()
    {
        try {
            ...
            $tBchCode           = $this->input->post('tBchCode');
            //$tBchCode           = $this->Salemachinedevice_model->FSaMGetBch($tPosCode); // รหัสจุดขาย
            $aSysPosHw          = $this->Salemachinedevice_model->FSaMGetSysPosHW();
            $aSysPrinter        = $this->Salemachinedevice_model->FSaMGetSysPrinter();
    ...
    public function FSvCPHWEditPage()
    {
        try {
            ...
            //$tBchCode           = $this->Salemachinedevice_model->FSaMGetBch($tPosCode); // รหัสจุดขาย
            $aPhwData       = $this->Salemachinedevice_model->FSaMPHWGetDataByID($aData);
            $aDataSaleMachineDevice   = array(
                'nStaAddOrEdit' => 1,
                'tPosCode'      => $tPosCode,
                'aPhwData'      => $aPhwData,
    ...
    public function FSoCPHWAddEvent()
    {
        ...
        // }
        $nLastSeq   = $this->Salemachinedevice_model->FSaMLastSeqByShwCode($tPosCode);
        $nLastSeq   = $nLastSeq + 1;
        $aDataSaleMachineDevice   = array(
            'FTPhwCode'        => $tPhwCode,
            'FTPosCode'        => $tPosCode,
        ...
        $nDupPli =  $this->Salemachinedevice_model->FSnMPHWCheckDuplicate($aDataSaleMachineDevice['FTPhwCode'], $aDataSaleMachineDevice['FTPosCode'], $aDataSaleMachineDevice['FTBchCode']);
        if ($nDupPli == 0) {
            $this->db->trans_begin();
            $this->Salemachinedevice_model->FSaMPHWAddUpdateMaster($aDataSaleMachineDevice);
    ...
    public function FSoCPHWEditEvent()
    {
        ...
        //     $tShwCode   = $this->input->post('oetShwCode');
        //     $aLastSeq   = $this->Salemachinedevice_model->FSaMLastSeqByShwCode($tPhwCode,$tPosCode,$tShwCode);
        ...
        //     $aStaPhwMaster  = $this->Salemachinedevice_model->FSaMPHWAddUpdateMaster($aDataSaleMachineDevice);
        //     $aStaPhwLang    = $this->Salemachinedevice_model->FSaMPHWAddUpdateLang($aDataSaleMachineDevice);
        //     if($this->db->trans_status() === false){
        //         $this->db->trans_rollback();
        //         $aReturn = array(
        //             'nStaEvent'    => '900',
    ...
    public function FSoCPHWDeleteEvent()
    {
        ...
        $aResDel    = $this->Salemachinedevice_model->FSaMPHWDelAll($aDataMaster);
        $nNumRowPhw = $this->Salemachinedevice_model->FSnMPHWGetAllNumRow($tPosCode);
        if ($nNumRowPhw !== false) {
            $aReturn    = array(
    ...
    public function FSoCPHWCheckInputGenCode()
    {
        ...
        $tCheck = $this->Salemachinedevice_model->FSaMCheckInputGenCode($aDataGenCode);
        $nNum = $tCheck[0]->nNum;
        if ($nNum != '0') {
            $nStatus = '1';
