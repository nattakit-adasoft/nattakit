...
class Setprinter_controller extends MX_Controller {

    public function __construct(){
        parent::__construct ();
        $this->load->model('pos/setprinter/Setprinter_model');
    }
    ...
    public function FSvCSPRDataList(){
        ...
        $tAPIReq    = "";
        $tMethodReq = "GET";
        $aResList   = $this->Setprinter_model->FSaMSPRList($tAPIReq, $tMethodReq, $aData);
        $aGenTable  = array(
            'aDataList'     => $aResList,
            'nPage'         => $nPage,
    ...
    public function FSvCSPREditPage(){
        ...
        $tAPIReq        = "";
        $tMethodReq     = "GET";
        $aSpnData       = $this->Setprinter_model->FSaMSPRSearchByID($tAPIReq, $tMethodReq, $aData);
    ...
    public function FSaCSPRAddEvent(){
        try{
            ...
            $oCountDup  = $this->Setprinter_model->FSoMSPNCheckDuplicate($aDataMaster['FTPrnCode']);
            $nStaDup    = $oCountDup[0]->counts;


            if($nStaDup == 0){
                $this->db->trans_begin();
                $aStaRsnMaster  = $this->Setprinter_model->FSaMSPRAddUpdateMaster($aDataMaster);
                $aStaRsnLang    = $this->Setprinter_model->FSaMSPRAddUpdateLang($aDataMaster);
    ...
    public function FSoCSPRDelete(){
        ...
        $aResDel = $this->Setprinter_model->FSnMSPRDel($aDataMaster);
        $aReturn = array(
