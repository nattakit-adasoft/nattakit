...
class Posregister_controller extends MX_Controller {

    public function __construct() {
        parent::__construct ();
        $this->load->model('pos/posregister/Posregister_model');
    ...
    public function FSvPRGPageList(){
        $aOption = $this->Posregister_model->FSaMPRGGetStaApp();
        $aReturn = array(
            'aOption' => $aOption
        );
    ...
    public function FSvPRGGetTable(){
        try{
            ...
            $aResultDatatable =  $this->Posregister_model->FSaMPRGDataTable($aData);
        ...
        try{
            ...
            if(isset($tMacAddr) != empty($tMacAddr)){
                for($i=0; $i < count($tMacAddr); $i++){
                    ...
                    $this->Posregister_model->FSaMPRGAddUpdateMaster($aDataInsert, $tMacAddr[$i]['ohdMacAddr'], $tEncPassword[$i]['tEncPassword'], $tBchCode[$i]['ohdBchCode'], $tPosCode[$i]['ohdPosCode'], $tPrgDate[$i]['ohdPrgDate']); 
                }
            }
            ...
            if(isset($tPosCode) != empty($tPosCode)){
                for($n=0; $n < count($tPosCode); $n++){
                    ...
                    $this->Posregister_model->FSaMPRGAddUpdateMasterPOS($aDataInsert, $tPosCode[$n]['ohdPosCode'], $tEncPassword[$n]['tEncPassword'], $tBchCode[$n]['ohdBchCode']);
                }
            }
          ...
          // ***********************************************************************************
            ...
            if(isset($tCancelMacAddr) != empty($tCancelMacAddr)){
                for($j=0; $j < count($tCancelMacAddr); $j++){
                    ...
                    $this->Posregister_model->FSaMPRGCancelUpdateMaster($aDataCancelUpdate, $tCancelMacAddr[$j]['ohdCancelMacAddr'], $tCancelEncPassword[$j]['tEncryptCancelPassword'], $tCancelBchCode[$j]['ohdCancelBchCode'], $tCancelPosCode[$j]['ohdCancelPosCode']); 
                }
            }
            ...
            if(isset($tCancelPosCode) != empty($tCancelPosCode)){
                for($k=0; $k < count($tCancelPosCode); $k++){
                    ...
                    $this->Posregister_model->FSaMPRGCancelUpdateMasterPOS($aDataCancelUpdate, $tCancelPosCode[$k]['ohdCancelPosCode'], $tCancelEncPassword[$k]['tEncryptCancelPassword'], $tCancelBchCode[$k]['ohdCancelBchCode']);
                }
            }
       }catch(Exception $Error){
           echo $Error;
