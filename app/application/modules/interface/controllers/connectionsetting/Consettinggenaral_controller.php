...
class Consettinggenaral_controller extends MX_Controller {
    public function __construct(){
        parent::__construct ();
        date_default_timezone_set("Asia/Bangkok");
        $this->load->model('interface/connectionsetting/Connsetgenaral_model');
    }
    ...
    public function FSvSETPageEdit(){
        try{
            ...
            $aResult  = $this->Connsetgenaral_model->FSaMSettingGenCheckID($aData);

            $aDataApi = array(
                'tApiCode' =>   @$aResult['raItems']['FTApiCode'],
            );

            $aDataApi = $this->Connsetgenaral_model->FSaMSettingGenDataList($aData, $aDataApi);
    ...
    public function FSvSETPageEditApiAuth(){
        try{
            ...
            $aResComp  = $this->Connsetgenaral_model->FSaMSettingGenCompCode();
            ...
            $aResultApi  = $this->Connsetgenaral_model->FSaMSetGenaralCheckID($aData);
    ...
    public function FSvSETDataList(){
        ...
        $aResList       = $this->Connsetgenaral_model->FSaMConnSetGenDataList($aData);
        $aReslistApi    = $this->Connsetgenaral_model->FSaMConnSetGenDataListApi($aData);
        ...
        try{
            ...
            if(count($aUsrValue) >= 1){
                for($i=0; $i<count($aUsrValue); $i++){
                    ...
                    $this->db->trans_begin();
                    //Update TLKMConfig
                    $aResult   = $this->Connsetgenaral_model->FSaMCSSUpdate($aUpdate);
            ...
            if(count($aSeqGrp) >= 1){
                for($j=0;$j<count($aSeqGrp); $j++){
                    ...
                    if($tSeqold[$j] == $aSeqGrp[$j]){
                        ...
                        $ChkDataDup  = $this->Connsetgenaral_model->FSaMCheckCodeDup($aUpdateApi); 
                    }
                    ...
                    if($ChkDataDup['rtCode'] == 1){
                        ...
                        $aReturn    = $this->Connsetgenaral_model->FSaMCSSUpdateApi($aUpdateApi);
                    }
    ...
    public function FSvSETEventAuthorAdd(){
        try{
            ...
            $aStaDup = $this->Connsetgenaral_model->FSaMCSGSpcCheckDupplicate($aDataMaster);

            if($aStaDup['rtCode']==1){
            $this->db->trans_begin();
            $aStaEventAdd  = $this->Connsetgenaral_model->FSaMSetAddUpdateMaster($aDataMaster);

            // Insert TCNMTxnAPI  (FDLastUpdOn) ** ถ้ามีการ update ที่ TCNMTxnSpcAPI ต้อง update ที่ TCNMTxnAPI ด้วย
            $this->Connsetgenaral_model->FSaMSetAddUpdateMasterTxnApi($aDataMaster);
    ...
    public function FSaSETDeleteEvent(){
        ...
        $aResDel        = $this->Connsetgenaral_model->FSnMSetGenaralDel($aDataMaster);
        $nNumRowRsnLoc  = $this->Connsetgenaral_model->FSnMLOCGetAllNumRow();
