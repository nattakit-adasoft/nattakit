<script>
    $(document).ready(function () {
        ...
        $(".xWAddGroup").validate({
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url(); ?>EticketAgency/AddGroupAjax",
                    data: $(".xWAddGroup").serialize(),
                    cache: false,
                    success: function (msg) {
                        var nDataId = $('.xWBtnSaveActive').data('id');
                        if (nDataId == '1') {
                            JSxCallPage('<?= base_url() ?>EticketAgency/EditGroup/' + msg)
                        } else if (nDataId == '2') {
 ...
 <div class="main-menu">
    <form action="" method="post" autocorrect="off" autocapitalize="off" autocomplete="off" class="xWAddGroup"> 
                ...
                <div class="xCNBchVMaster">
                    <div class="col-xs-8 col-md-8">
                        <ol id="oliMenuNav" class="breadcrumb">
                            ...
                            <li  class="xCNLinkClick"><?= language('ticket/agency/agency', 'tAddAgencyGroup') ?></li>
                            </ol>       
                            </div>
                    ...
                    <div class="input100 validate-input" data-validate="<?= language('ticket/agency/agency', 'tPleaseEnterAgencyGroupName') ?>">
                        <div class="form-group">
                            <label class="xCNLabelFrm"><span style="color:red">*</span><?= language('ticket/agency/agency', 'tAgencyGroupName') ?></label>
							    <input type="text" class="form-control" maxlength="50" id="oetFTAggName" name="oetFTAggName" data-validate="<?= language('ticket/agency/agency', 'tPleaseEnterAgencyGroupName') ?>">
                                </div>
                            </div>
                        </div> 
                    </div> 
