    ...
    $(document).ready(function () {
        $(".xWAddType").validate({
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url(); ?>EticketAgency/AddTypeAjax",
                    data: $(".xWAddType").serialize(),
                    cache: false,
                    success: function (msg) {
                        var nDataId = $('.xWBtnSaveActive').data('id');
                        if (nDataId == '1') {
                            JSxCallPage('<?= base_url() ?>EticketAgency/EditType/'+msg+'');                            
                        } else if (nDataId == '2') {
...
<div class="main-menu">
    <form action="" method="post" autocorrect="off" autocapitalize="off" autocomplete="off" class="xWAddType">
                ...
                <div class="xCNBchVMaster">
                    <div class="col-xs-8 col-md-8">
                        <ol id="oliMenuNav" class="breadcrumb">
                            ...
                            <li  class="xCNLinkClick"><?= language('ticket/agency/agency', 'tAddAgencyType') ?></li>
                            </ol>       
                            </div>
                    ...
                    <div class="input100 validate-input" data-validate="<?= language('ticket/agency/agency', 'tPleaseEnterAgencyTypeName') ?>"   >
                        <div class="form-group">
                            <label class="xCNLabelFrm"><span style="color:red">*</span><?= language('ticket/agency/agency', 'tAgencyGroupName') ?></label>
							    <input class="form-control" type="text" name="oetFTAtyName" id="oetFTAtyName" data-validate="<?= language('ticket/agency/agency', 'tAgencyGroupName') ?>">
                                </div>
                            </div>
                        </div> 
                    </div> 
