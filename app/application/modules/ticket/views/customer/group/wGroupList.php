...
<div class="table-responsive">
	...
	<tbody>		
		<?php  foreach ($oCstGpList as $key => $tValue) : ?>	
			<tr class="xCNTextDetail2" data-code="<?= $tValue->FNCgpID; ?>" data-name="<?= $tValue->FTCgpName ?>">
				<td class="text-center">
					<label class="fancy-checkbox">
						<input id="ocbListItem<?=$key?>" type="checkbox" data-name="<?= $tValue->FTCgpName ?>" value="<?=$tValue->FNCgpID;?>" class="ocbListItem" name="ocbListItem[]">
							<span>&nbsp;</span>
						</label>
					</td>
				<td>
			<?php if($tValue->FTCgpName): ?>
				<?= $tValue->FTCgpName ?>
					<?php else: ?>
			...
			<?php if ($oAuthen['tAutStaDelete'] == '1'): ?>
              	<td class="text-center">		
					<!-- เปลี่ยน -->
						<img class="xCNIconTable" src="<?php echo  base_url().'application/modules/common/assets/images/icons/delete.png'?>" onclick="FSxCstGroupDel('<?= $nPageNo ?>','<?= $tValue->FNCgpID ?>', '<?= $tValue->FTCgpName ?>')">						   			
						</td>
              		<?php endif; ?>
			    <?php if ($oAuthen['tAutStaEdit'] == '1'): ?>
            <td class="text-center">
				<!-- เปลี่ยน -->
					<img class="xCNIconTable" src="<?php echo  base_url().'application/modules/common/assets/images/icons/edit.png'?>" onclick="JSxCallPage('<?php echo base_url()?>EticketCustomer/EditGroup/<?= $tValue->FNCgpID ?>');">
					</td>
                <?php endif; ?>
			</tr>
		<?php endforeach ?>		
