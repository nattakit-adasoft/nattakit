	...
	$(document).ready(function () {
		$(".xWEditGroupType").validate({
			...
			submitHandler: function (form) {
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>EticketCustomer/EditGroupAjax",
					data: $(".xWEditGroupType").serialize(),
					cache: false,
					success: function (msg) {
						var nDataId = $('.xWBtnSaveActive').data('id');
						if (nDataId == '1') {
							JSxCallPage('<?= base_url()?>EticketCustomer/EditGroup/<?= $oGroup[0]->FNCgpID ?>');
						} else if(nDataId == '2') {
...
<div class="main-menu">
	<form action="" method="post" autocorrect="off" autocapitalize="off" autocomplete="off" class="xWEditGroupType">
                ...
                <div class="xCNBchVMaster">
                    <div class="col-xs-8 col-md-8">
                        <ol id="oliMenuNav" class="breadcrumb">
                            ...
                            <li  class="xCNLinkClick"><?= language('ticket/customer/customer', 'tEditCustomerGroup') ?></li>
                            </ol>       
                            </div>
...
<input type="hidden" name="ohdFNCgpID" id="ohdFNCgpID" value="<?= $oGroup[0]->FNCgpID ?>" />	 
					...
					<div class="col-xs-12 col-md-5 col-lg-5">
						<div class="input100 validate-input" data-validate="<?= language('ticket/customer/customer', 'tPleaseEnterCustomerCategoryName') ?>">
							<div class="form-group">
								<label class="xCNLabelFrm"><?= language('ticket/customer/customer', 'tCustomerGroupName') ?></label>
									<input class="form-control" type="text" name="oetFTCgpName" id="oetFTCgpName" value="<?= $oGroup[0]->FTCgpName ?>" data-validate="<?= language('ticket/customer/customer', 'tCustomerGroupName') ?>">
