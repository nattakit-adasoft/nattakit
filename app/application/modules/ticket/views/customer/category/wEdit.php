	...
	$(document).ready(function () {
		$(".xWEditCustomerType").validate({
			...
			submitHandler: function (form) {
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>EticketCustomer/EditCategoryAjax",
					data: $(".xWEditCustomerType").serialize(),
					cache: false,
					success: function (msg) {
						var nDataId = $('.xWBtnSaveActive').data('id');
						if (nDataId == '1') {
							JSxCallPage('<?= base_url()?>EticketCustomer/EditCategory/<?= $oCategory[0]->FNCtyID ?>');
						} else if(nDataId == '2') {
...
<div class="main-menu">
	<form action="" method="post" autocorrect="off" autocapitalize="off" autocomplete="off" class="xWEditCustomerType">
                ...
                <div class="xCNBchVMaster">
                    <div class="col-xs-8 col-md-8">
                        <ol id="oliMenuNav" class="breadcrumb">
                            ...
                            <li  class="xCNLinkClick"><?= language('ticket/customer/customer', 'tEditCustomerCategory') ?></li>
                            </ol>       
                            </div>
...
<input type="hidden" name="ohdFNCtyID" id="ohdFNCtyID" value="<?= $oCategory[0]->FNCtyID ?>" /> 
                    ...
                    <div class="col-xs-12 col-md-5 col-lg-5">
						<div class="input100 validate-input" data-validate="<?= language('ticket/customer/customer', 'tPleaseEnterCustomerCategoryName') ?>">
                            <div class="form-group">
                                <label class="xCNLabelFrm"><?= language('ticket/customer/customer', 'tCustomerCategoryName') ?></label>
                                    <input class="form-control" type="text" name="oetFTCtyName" id="oetFTCtyName" value="<?= $oCategory[0]->FTCtyName ?>" data-validate="<?= language('ticket/customer/customer', 'tPleaseEnterCustomerCategoryName') ?>">
                                    </div>
                                </div>
                            </div> 
                        </div> 
