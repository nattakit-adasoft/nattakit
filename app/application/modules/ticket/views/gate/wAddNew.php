	...
	$(function(){
		$("#xWAddGate").validate({
			...
			submitHandler: function (form) {
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url(); ?>EticketAddGateAjax",
					data: $("#xWAddGate").serialize(),
					cache: false,
					success: function (msg) {
						var nDataId = $('.xWBtnSaveActive').data('id');
						if (nDataId == '1') {
							JSxCallPage('<?php echo base_url()?>EticketEditGateNew/<?php echo $nLocID; ?>/'+msg);
						} else if(nDataId == '2') {
...
>>>>>>> 81bf17ac4472cccb47e9b8a7fad9daddc4181c76
                            ...
                            <li id="oliBCHTitle" class="xCNLinkClick"><?= language('ticket/gate/gate', 'tAddGate') ?></li>
                        </ol>
                    </div>
                </div>
            </div>
	...
	<form action="" method="post" autocorrect="off" autocapitalize="off" autocomplete="off" id="xWAddGate">
					...
					<div class="row">
						<div class="col-md-6 col-sm-6 col-xs-12">
								...
								<div data-validate="<?= language('ticket/gate/gate', 'tPleaseSelectZone') ?>">
									...
									<select class="selectpicker form-control" id="ocmFNZneID" name="ocmFNZneID" title="<?= language('ticket/gate/gate', 'tSelectZone') ?>">
										...
										<?php else : ?>
											...
											<option value="<?= $tValue->FNZneID ?>"><?= $tValue->FTZneName ?></option>
											<?php  endforeach; ?>
										<?php endif; ?>
									</select>
								</div>
							...
							<div class="form-group">
								...
								<input type="text" class="form-control xCNInputWithoutSingleQuote" id="oetFTGteName" name="oetFTGteName">
							</div>
						</div>
						<div class="col-md-6 col-sm-6 col-xs-12">
