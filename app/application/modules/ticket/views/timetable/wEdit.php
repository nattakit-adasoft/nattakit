<script>
	$(document).ready(function () {
		$(".xWEditTimeTable").validate({
			...
			submitHandler: function (form) {
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>EticketTimeTable/EditTimeTableAjax",
					data: $(".xWEditTimeTable").serialize(),
					cache: false,
					success: function (msg) {						
						var nDataId = $('.xWBtnSaveActive').data('id');
						if (nDataId == '1') {
							JSxCallPage('<?php echo base_url()?>EticketTimeTable/EditTimeTable/<?= $oShow[0]->FNTmhID; ?>');
						} else if(nDataId == '2') {
	...
	<form action="" method="post" autocorrect="off" autocapitalize="off" autocomplete="off" class="xWEditTimeTable">	
		<div class="main-menu">
					...
					<div class="xCNBchVMaster">
						<div class="col-xs-8 col-md-8">
							<ol id="oliMenuNav" class="breadcrumb">
								...
								<li class="xCNLinkClick"><?= language('ticket/timetable/timetable', 'tEditTimeTable') ?></li>
							</ol>
						</div>
					...
					<div class="row">			
						<div class="col-md-8 col-sm-4 col-xs-12">
							<div class="form-group" >
								...
								<input type="text" class="form-control" id="oetFTTmhName" name="oetFTTmhName" data-validate="<?= language('ticket/timetable/timetable', 'tPleaseEnterNameTimeTable') ?>" value="<?= $oShow[0]->FTTmhName; ?>">
							</div>
							<div class="form-group">
								<label class="xCNLabelFrm"><?= language('ticket/zone/zone', 'tRemarks') ?></label>				
								<textarea class="form-control" maxlength="100" id="otaFTTmhRmk" name="otaFTTmhRmk"><?= $oShow[0]->FTTmhRmk; ?></textarea>
							</div>
							<div class="form-group">
								<label class="pull-left" style="font-weight: normal;">
									<input type="checkbox" <?php if ($oShow[0]->FTTmhStaActive == '1') {echo ' checked="checked"';} elseif($oShow[0]->FTTmhStaActive == '2') {echo '';}?> value="1" style="float: left; margin-right: 5px;" name="ocbFTTmhStaActive"> <?= language('ticket/timetable/timetable', 'tOpening') ?>
								</label>
							</div>
						</div>
