<script>
	$(function () {
		$("#oEditFloor").validate({			
			...
			submitHandler: function (form) {
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url(); ?>EticketEditLevelAjax",
					data: $("#oEditFloor").serialize(),
					cache: false,
					success: function (msg) {                        
						var nDataId = $('.xWBtnSaveActive').data('id');
						if (nDataId == '1') {
							JSxCallPage('<?= base_url()?>EticketEditLevelNew/<?= $nLocID ?>/<?= $nLvlID ?>');
						} else if(nDataId == '2') {
				...
				<div class="xCNBchVMaster">
					<div class="col-xs-8 col-md-8">
						<ol id="oliMenuNav" class="breadcrumb">
							...
							<li  class="xCNLinkClick"><?= language('ticket/level/level', 'tEditLevel') ?></li>
						</ol>
					</div>
				</div>
			</div>
	...
	<form action="" method="post" autocorrect="off" autocapitalize="off" autocomplete="off" id="oEditFloor">
					...
					<div class="container-fluid">
						...
						<div class="row">
							<div class="col-md-6 col-sm-6 col-xs-12">
								<input type="hidden" name="ohdFNLevID" id="ohdFNLevID" value="<?= $nLvlID ?>">								
								<div class="form-group">
									<label class="xCNLabelFrm"><?= language('ticket/level/level', 'tNameLevel') ?></label>
									<input type="text" class="form-control xCNInputWithoutSingleQuote" id="oetFTLevName" name="oetFTLevName" value="<?= @$oEdit[0]->FTLevName; ?>">
								</div>
							</div>
