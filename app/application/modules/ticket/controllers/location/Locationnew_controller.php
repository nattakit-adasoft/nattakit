...
class Locationnew_controller extends MX_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model('ticket/location/mLocationNew', 'mLocation');
    ...
    public function FSxCLocLocation($nID) {
        ...
        $this->load->view('ticket/location/wLocationNew', array(
            'oLocL' => $oLocL,
            'oArea' => $oArea,
            'oModelImg' => $oModelImg,
            'oAuthen' => $oAuthen,
    ...
    public function FSxCLOCList() {
        ...
        $oAuthen = FCNaHCheckAlwFunc('EticketBranch');
        $this->load->view('ticket/location/wLocListNew', array(
            'oLocList'  => $oLocList,
            'oAuthen'   => $oAuthen,
            'nPageNo'   => $nPageNo,
            'nParkId'   => $tParkId
    ...
    public function FSxCLOCAdd($tID) {
        ...
        $this->load->view('ticket/location/wAddNew', array(
            'aArea' => $oArea,
            'aProvince' => $oProvince,
            'aDistrict' => $oDistrict,
            'aModel' => $oModel,
    ...
    public function FSxCLocAddAjax(){
        ...
        if ($this->input->post('ocmSelectFNPmoID')) {
            ...
            $tTimeClosing = $this->input->post('oetFTLocTimeClosing');
            ...
            foreach ($tFNPmoID as $key => $tValue) {
                $tData = array(
                    ...
                    'FTLocTimeClosing' => $tTimeClosing, // เวลาปิดทำการ เช่น 20:00:00
                );
    ...
    public function FSxCLOCEdit($nID, $nPrk) {
        ...
        $this->load->view('ticket/location/wEditNew', array(
            'aArea' => $oArea,
            'aProvince' => $oProvince,
            'aDistrict' => $oDistrict,
            'aLocModel' => $oLocModel,
    ...
    public function FSxCLocDeleteLocation() {
        if ($this->input->post('tId')) {
            ...
            foreach ($aCode as $key => $tIdLoc) {
                $oCount = $this->Location_model->FSxMLocDelLoc($tIdLoc);
                if ($oCount == 0) {
                    $aDeleteImage = array(
                        'tModuleName'  => 'ticket',
                        'tImgFolder'   => 'ticketlocation',
    ...
    public function FSxCLOCLoadArea() {
        if ($this->input->post('nId')) {
            ...
            $this->load->view('ticket/location/wArea', array(
                'oRow' => $oRow
            ));
        }
    }
