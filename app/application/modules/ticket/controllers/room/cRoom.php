...
class cRoom extends CI_Controller {
	public function __construct() {
		parent::__construct ();
		$this->load->model ( 'room/mRoom', 'mRoom' );
	...
	public function FSxCROM($nLocID, $nZneID) {
		...
		$this->load->view ( 'room/wRoom', array (
				'oAuthen' => $oAuthen,
				'oHeader' => $oHeader,
				'oArea' => $oArea,
				'nLocID' => $nLocID,
	...
	public function FSxCROMList() {
		...
		$oAuthen = $this->mRoom->FSxMROMAuthen ();
		$this->load->view ( 'room/wRoomList', array (
				'oAuthen' => $oAuthen,
				'oRomList' => $oRomList 
		) );
	}
	...
	public function FSxCROMAdd($nLocID, $nZneID) {
		...
		$this->load->view ( 'room/wAdd', array (
				'nLocID' => $nLocID,
				'nZneID' => $nZneID,
				'oLev' => $oLev,
				'oTcg' => $oTcg,
	...
	public function FSxCROMEdit($nRoomID, $nLocID) {
		...
		$oTcg = $this->mRoom->FSxMROMTcg ( $nLocID );
		$this->load->view ( 'room/wEdit', array (
				'nRoomID' => $nRoomID,
				'oLev' => $oLev,
				'oEdit' => $oEdit,
				'oTcg' => $oTcg 
