...
class Bookbank_controller extends MX_Controller {

    public function __construct(){
        date_default_timezone_set("Asia/Bangkok");
        parent::__construct ();
        $this->load->model('bookbank/bookbank/Bookbank_model');
    }
    ...
    public function FSxCBBKDataTable(){
        ...
        $aResList   = $this->Bookbank_model->FSaMBBKList($aData);
        $aGenTable  = array(
            'aAlwEvent'     => $aAlwEvent,
            'aDataList'     => $aResList,
            'nPage'         => $nPage,
    ...
    public function FSaCBBKAddEvent(){
        try{
            ...
            // $oCountDup  = $this->Bookbank_model->FSnMBbkCheckDuplicate($aDataMaster['FTBbkCode'], $aDataMaster['FTBchCode']);
            // $nStaDup    = $oCountDup[0]->counts;
            $bBbkIsDuplicate = $this->Bookbank_model->FSnMBbkCheckDuplicate($aDataMaster['FTBbkCode'], $aDataMaster['FTBchCode']);
            // if($nStaDup == 0){
            if(!$bBbkIsDuplicate){
                $this->db->trans_begin();
                $aStaEventMaster  = $this->Bookbank_model->FSaMBBKAddUpdateMaster($aDataMaster);
                $aStaEventLang    = $this->Bookbank_model->FSaMBBKAddUpdateLang($aDataMaster);
    ...
    public function FSvCBBKEditPage(){
        ...
        $aResult           = $this->Bookbank_model->FSaMBBKSearchByID($aData);
        $aDataEdit  = [
            'aResult'               => $aResult,
    ...
    public function FSaCBBKDeleteEvent(){
        ...
        $aResDel        = $this->Bookbank_model->FSnMBBKDel($aDataMaster);
        $nNumRowBbk     = $this->Bookbank_model->FSnMBBKGetAllNumRow();
