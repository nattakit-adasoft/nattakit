...
class Customerlevel_controller extends MX_Controller {

    public function __construct(){
        parent::__construct();
        $this->load->model('customer/customerlevel/Customerlevel_model');
    ...
    public function FSvCstLevDataList(){
        ...
        $tAPIReq = "";
        $tMethodReq = "GET";
        $aResList = $this->Customerlevel_model->FSaMCstLevList($tAPIReq, $tMethodReq, $aData);
        $aGenTable = array(
            'aDataList' => $aResList,
            'nPage' => $nPage,
            'tSearchAll' => $tSearchAll
    ...
    public function FSvCstLevEditPage(){
        ...
        $tAPIReq        = "";
        $tMethodReq     = "GET";
        $aCstLevData       = $this->Customerlevel_model->FSaMCstLevSearchByID($tAPIReq,$tMethodReq,$aData);
    ...
    public function FSaCstLevAddEvent(){
        try{
            ...
            $oCountDup  = $this->Customerlevel_model->FSoMCstLevCheckDuplicate($aDataMaster['FTCstLevCode']);
            $nStaDup    = $oCountDup[0]->counts;

            if($nStaDup == 0){
                $this->db->trans_begin();
                $aStaCstLevMaster  = $this->Customerlevel_model->FSaMCstLevAddUpdateMaster($aDataMaster);
                $aStaCstLevLang    = $this->Customerlevel_model->FSaMCstLevAddUpdateLang($aDataMaster);
    ...
    public function FSaCstLevDeleteEvent(){
        ...
        $aResDel = $this->Customerlevel_model->FSnMCstLevDel($aDataMaster);
        $aReturn = array(
    ...
    public function FStCstLevUniqueValidate($tSelect = ''){
        ...
        if($this->input->is_ajax_request()){ // Request check
            if($tSelect == 'CstLevCode'){
                ...
                $tCstLevCode = $this->input->post('tCstLevCode');
                $oCustomerLevel = $this->Customerlevel_model->FSoMCstLevCheckDuplicate($tCstLevCode);
    ...
    public function FSoCstLevDeleteMulti(){
        ...
        foreach($aCstLevCode as $oCstLevCode){
            ...
            $this->Customerlevel_model->FSnMCstLevDel($aCstLev);
        }
