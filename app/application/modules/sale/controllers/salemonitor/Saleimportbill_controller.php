...
class Saleimportbill_controller extends MX_Controller {

    public function __construct() {
        $this->load->model('sale/salemonitor/Saleimportbill_model');
        parent::__construct();
    }
    ...
    // Return Type : View
        public function FSaCIMPUploadFile(){
            try{
                ...
                // echo $tXshDocNo;
                $this->db->trans_begin();
                $this->Saleimportbill_model->FSaMIMPAddUpdateBill($aBillFile);
                $this->Saleimportbill_model->FSaMIMPUpdateSalHDSpc($tXshDocNo);
                $this->Saleimportbill_model->FSaMIMPUpdateSalDTSpc($tXshDocNo);
    ...
    public function FSvCIMPLoadDatatable(){
        ...
        if($tDocumentNumber == '' || $tDocumentNumber == null){
            ...
            $aGetDT     = $this->Saleimportbill_model->FSaMIMPGetDT($aWhere);
            $aGetHD     = $this->Saleimportbill_model->FSaMIMPGetHD($aWhere);
        }

        // $aGetDT     = $this->Taxinvoice_model->FSaMTAXGetDT($aWhere);
        // $aGetHD     = $this->Taxinvoice_model->FSaMTAXGetHD($aWhere);
    ...
    public function FSaCIMPInsertBillData(){
        ...
        $this->db->trans_begin();
        $this->Saleimportbill_model->FSaMIMPTPSTSalHD($aDataWhere);
        $this->Saleimportbill_model->FSaMIMPTPSTSalDT($aDataWhere);
        $this->Saleimportbill_model->FSaMIMPTPSTSalDTDis($aDataWhere);
        $this->Saleimportbill_model->FSaMIMPTPSTSalDTPmt($aDataWhere);
        $this->Saleimportbill_model->FSaMIMPTPSTSalHDCst($aDataWhere);
        $this->Saleimportbill_model->FSaMIMPTPSTSalHDDis($aDataWhere);
        $this->Saleimportbill_model->FSaMIMPTPSTSalPD($aDataWhere);
        $this->Saleimportbill_model->FSaMIMPTPSTSalRC($aDataWhere);
        $this->Saleimportbill_model->FSaMIMPTPSTSalRD($aDataWhere);
        $this->Saleimportbill_model->FSaMIMPTCNTMemTxnRedeem($aDataWhere);
        $this->Saleimportbill_model->FSaMIMPTCNTMemTxnSale($aDataWhere);
         if($this->db->trans_status() === false){
                $this->db->trans_rollback();
                $aDataStaReturn = array(
                    'nStaEvent'    => '900',
