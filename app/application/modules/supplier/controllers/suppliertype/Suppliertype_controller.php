...
class Suppliertype_controller extends MX_Controller {
    public function __construct(){
        parent::__construct ();
        $this->load->model('supplier/suppliertype/Suppliertype_model');
    ...
    public function FSvCSTYDataList(){
        try{
            ...
            $aStyDataList   = $this->Suppliertype_model->FSaMSTYList($aData); 
            $aGenTable  = array(
    ...
    public function FSvCSTYEditPage(){
        try{
            ...
            $aStyData       = $this->Suppliertype_model->FSaMSTYGetDataByID($aData);
            $aDataSupplierType   = array(
                'nStaAddOrEdit' => 1,
                'aStyData'      => $aStyData
            );
    ...
    public function FSoCSTYAddEvent(){ 
        try{
            ...
            $oCountDup      = $this->Suppliertype_model->FSnMSTYCheckDuplicate($aDataSupplierType['FTStyCode']);
            $nStaDup        = $oCountDup['counts'];

            if($oCountDup !== FALSE && $nStaDup == 0){
                $this->db->trans_begin();
                $aStaStyMaster  = $this->Suppliertype_model->FSaMSTYAddUpdateMaster($aDataSupplierType);
                $aStaStyLang    = $this->Suppliertype_model->FSaMSTYAddUpdateLang($aDataSupplierType);
    ...
    public function FSoCSTYDeleteEvent(){
        ...
        $aResDel    = $this->Suppliertype_model->FSaMSTYDelAll($aDataMaster);
        $aReturn    = array(
