...
class Product_controller extends CI_Controller {
    ...
    public function __construct(){
        parent::__construct ();
        $this->load->model('pos5/supplier/Product_model');
    }
    ...
    public function FSvCPDTDataList(){
        try{
            ...
            $aDataList   = $this->Product_model->FSaMPDTList($aData);
            $aGenTable  = array(
                'aDataList'  => $aDataList,
                'nPage'         => $nPage,
                'tSplCode'         => $tSplCode,
    ...
    public function FSvCPDTEditPage(){
        try{
            ...
            // print_r($aData);
            // exit;
            $aDataByID = $this->Product_model->FSaMPDTGetDataByID($aData);
            $aVatList = FCNoHCallVatlist(); //-->Call Vat Active
            $aDataProduct  = array(
                'nStaAddOrEdit' => 1,
				'aVatRate'		=> $aVatList,
    ...
    public function FSoCPDTAddEvent(){
        try{
            ...
            // exit;
            $this->db->trans_begin();
            $aPdtCode = $this->Product_model->FSaMPDTGetPdtCode($aBarCode);
            // echo '<pre>';
            // print_r($aPdtCode);
            if(count($aPdtCode['raItem']) > 0){
                foreach($aPdtCode['raItem'] as $nKey => $aVal){
                    ...
                    $aChkDup = $this->Product_model->FSaMPDTChkDup($aDataMaster);
                    if($aChkDup['raItem'] < 1){
                        $this->Product_model->FSaMPDTAddMaster($aDataMaster);
                    }
                }
            }
    ...
    public function FSoCPDTEditEvent(){
        try{
            ...
            $this->db->trans_begin();
            $this->Product_model->FSaMPDTUpdateMaster($aDataMaster);
    ...
    public function FSoCPDTDeleteEvent(){
        try{
            ...
            $aResDel        = $this->Product_model->FSnMPDTDel($aDataMaster);
            $aReturn    = array(
