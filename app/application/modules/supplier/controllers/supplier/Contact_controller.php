...
class Contact_controller extends CI_Controller {
    ...
    public function __construct(){
        parent::__construct ();
        $this->load->model('pos5/supplier/Contact_model');
    }
    ...
    public function FSvCCTRDataList(){
        try{
            ...
            $aDataList   = $this->Contact_model->FSaMCTRList($aData);
            $aGenTable  = array(
                'aDataList'  => $aDataList,
                'nPage'         => $nPage,
                'tSearchAll'    => $tSearchAll
    ...
    public function FSvCCTREditPage(){
        try{
            ...
            // print_r($aData);
            // exit;
            $aDataByID = $this->Contact_model->FSaMCTRGetDataByID($aData);
            $aVatList = FCNoHCallVatlist(); //-->Call Vat Active
            $aDataContact  = array(
                'nStaAddOrEdit' => 1,
				'aVatRate'		=> $aVatList,
    ...
    public function FSoCCTRAddEvent(){
        try{
            ...
            $this->db->trans_begin();
            $this->Contact_model->FSaMCTRAddMaster($aDataMaster);
            $aLastSeq = $this->Contact_model->FSaMCTRGetLastSeq();
            // echo '<pre>';
            // print_r($this->Contact_model->FSaMCTRGetLastSeq());
            // exit;
            ...
            // print_r($aDataAd);
            // exit;
            $this->Contact_model->FSaMCTRAddDT($aDataAd);
    ...
    public function FSoCCTREditEvent(){
        try{
            ...
            $this->db->trans_begin();
            $this->Contact_model->FSaMCTRUpdateMaster($aDataMaster,$tCtrSeq);
            $this->Contact_model->FSaMCTRDT($aDataAd,$aPK);
    ...
    public function FSoCCTRDeleteEvent(){
        try{
            ...
            $aResDel        = $this->Contact_model->FSnMCTRDel($aDataMaster);
            $aReturn    = array(
