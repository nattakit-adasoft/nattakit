...
class Supplier_controller extends MX_Controller {

    public function __construct(){
        parent::__construct ();
        $this->load->model('supplier/supplier/Supplier_model');
    ...
    public function FSvCSPLDataList(){
        try{
            ...
            $aSplDataList   = $this->Supplier_model->FSaMSPLList($aData);
            $aGenTable  = array(
    ...
    public function FSvCSPLAddAddressPage(){
        try{
            ...
            $aSplData   = $this->Supplier_model->FSaMSPLAddType();
            $aDataSupplier = array(
                'nStaAddOrEdit'   => 99,
                'aData'           => $aSplData,
    ...
    public function FSvCSPLEditPage(){
        ...
        $aSplData   = $this->Supplier_model->FSaMSPLGetDataByID($aData);
    ...
    public function FSvCSPLEAddressEditPage(){
        try{
            ...
            // $aSplDataAddress = $this->Supplier_model->FSaMSPLGetDataAddress($aData);
            // $aVatList = FCNoHCallVatlist(); //-->Call Vat Active
            $aSplAddressData       = $this->Supplier_model->FSaMSPLGetAddressDataByID($aData);
            $aSplAddressSeparate   = $this->Supplier_model->FSnMSPLGetAddressData($aData);//จังหวัด อำเภอ ตำบล
    ...
    public function FSvCSPLEContactEditPage(){
        try{
            ...
            // $aVatList = FCNoHCallVatlist(); //-->Call Vat Active
            $aSplContactData       = $this->Supplier_model->FSaMSPLGetContactDataByID($aData);
            $aDataSupplier  = array(
                'nStaAddOrEdit' => 1,
    ...
    public function FSoCSPLAddressDataTable(){
        try{
            $tSplcode = $this->input->post('tSplCode');
            $aSplDataAddress = $this->Supplier_model->FSaMSPLGetDataAddress($tSplcode);
            $aSplAddress =array(
    ...
    public function FSoCSPLContactDataTable(){
        try{
            $tSplcode = $this->input->post('tSplCode');
            $aSplDataContact = $this->Supplier_model->FSaMSPLGetDataContact($tSplcode);
            $aSplContact =array(
    ...
    public function FSoCSPLAddEvent(){
        ...
        $oCountDup      = $this->Supplier_model->FSnMSPLCheckDuplicate($aDataMaster['FTSplCode']);
        $nStaDup        = $oCountDup['counts'];
        if($nStaDup == 0){
            $this->db->trans_begin();
            $this->Supplier_model->FSaMSPLAddMaster($aDataMaster);
            $this->Supplier_model->FSaMSPLAddLang($aDataLang);
            $this->Supplier_model->FSaMSPLAddDT($aDataCard,'TCNMSplCard');
            $this->Supplier_model->FSaMSPLAddDT($aDataCredit,'TCNMSplCredit');
    ...
    public function FSoCSPLEditEvent(){
        ...
        $this->db->trans_begin();
        $this->Supplier_model->FSaMSPLUpdateMaster($aDataMaster);
        $this->Supplier_model->FSaMSPLUpdateDT($aDataLang,'TCNMSpl_L');
        $this->Supplier_model->FSaMSPLUpdateDT($aDataCard,'TCNMSplCard');
        $this->Supplier_model->FSaMSPLUpdateDT($aDataCredit,'TCNMSplCredit');
        if($this->db->trans_status() === false){
            $this->db->trans_rollback();
            $aReturn = array(
                'nStaEvent'    => '900',
    ...
    public function FSoCSPLDeleteEvent(){
        try{
            ...
            $aResDel        = $this->Supplier_model->FSnMSPLDel($aDataMaster);
            if($aResDel['rtCode'] == 1){
                $aDeleteImage = array(
                    'tModuleName'   => 'supplier',
                    'tImgFolder'   => 'supplier',
    ...
    public function FSoCSPLAddressAddEvent(){
        ...
        $this->Supplier_model->FSaMSPLAddAddress($aData);
    ...
    public function FSoCSPLContactAddEvent(){
        ...
        $tTextConfirm = $this->Supplier_model->FSaMSPLAddContact($aData);
    }
    ...
    public function FSoCSPLAddressEditEvent(){
        ...
        $this->Supplier_model->FSaMSPLUpdateAddress($aData);
    ...
    public function FSoCSPLContactEditEvent(){
        ...
        $this->Supplier_model->FSaMSPLUpdateContact($aData);
    ...
    public function FSoCSPLAddressDeleteEvent(){
        try{
            ...
            $aResDel        = $this->Supplier_model->FSnMSPLAddressDel($aDataMaster);
    ...
    public function FSoCSPLContactDeleteEvent(){
        try{
            ...
            $aResDel        = $this->Supplier_model->FSnMSPLContactDel($aDataMaster);
