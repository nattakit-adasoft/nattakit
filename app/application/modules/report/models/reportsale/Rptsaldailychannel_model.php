...
class Rptsaldailychannel_model extends CI_Model
{
    /**
    ...
    private function FMaMRPTPagination($paDataWhere)
    {
        ...
        $tSQL = "   
            ...
            FROM TRPTSalDailyChannelTmp TMP WITH(NOLOCK)
            WHERE 1=1
    ...
    private function FMxMRPTSetPriorityGroup($paDataWhere)
    {
        ...
        $tSQL = "   
            UPDATE TRPTSalDailyChannelTmp SET 
            TRPTSalDailyChannelTmp.FNRowPartID = B.PartID
                FROM(
                    SELECT   
            ...
            WHERE TRPTSalDailyChannelTmp.FTRptRowSeq = B.FTRptRowSeq
            AND TRPTSalDailyChannelTmp.FTComName = '$tComName' 
            AND TRPTSalDailyChannelTmp.FTRptCode = '$tRptCode'
            AND TRPTSalDailyChannelTmp.FTUsrSession = '$tUsrSession'
            $tAppTypePos
        ";
        $this->db->query($tSQL);
    }
    ...
    public function FSaMGetDataReport($paDataWhere)
    {
        ...
        if ($nPage == $nTotalPage) {
            $tRptJoinFooter = " 
                ...
                FROM TRPTSalDailyChannelTmp WITH(NOLOCK)
        ...
        $tSQL   =   "   
            ...
            FROM (
                SELECT
                    ...
                    S.FCXshRnd_SumSup,
                    S.FCXshGrand_SumSup
                FROM TRPTSalDailyChannelTmp A WITH(NOLOCK)
                /* Calculate Misures */
                LEFT JOIN (
                    SELECT
    ...
    public function FSnMExecStoreReport($paDataFilter)
    {
        ...
        $tCallStore = "{CALL SP_RPTxPSSDailyChannel(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
        $aDataStore = array(
            'pnLngID' => $nLangID,
            'pnComName' => $tComName,
            'ptRptCode' => $tRptCode,
            ...
            'ptChnL'        => '',
            'ptChnF'        => $paDataFilter['tChannelFrom'],
            'ptChnT'        => $paDataFilter['tChannelTo'],
