...
class Rpttaxsalefull_model extends CI_Model {
    ...
    private function FMaMRPTPagination($paDataWhere){
        ...
        $tSQL = "   
            ...
            FROM TRPTPSTaxHDFullTmp TMP WITH(NOLOCK)
            WHERE 1=1
    ...
    private function FMxMRPTSetPriorityGroup($paDataWhere){
        ...
        $tSQL = "   
            UPDATE TRPTPSTaxHDFullTmp SET 
                TRPTPSTaxHDFullTmp.FNRowPartID = B.PartID
                FROM(
                    SELECT   
            ...
            WHERE TRPTPSTaxHDFullTmp.FTRptRowSeq = B.FTRptRowSeq
            AND TRPTPSTaxHDFullTmp.FTComName = '$tComName' 
            AND TRPTPSTaxHDFullTmp.FTRptCode = '$tRptCode'
            AND TRPTPSTaxHDFullTmp.FTUsrSession = '$tUsrSession'
            $tAppTypePos
        ";
        $this->db->query($tSQL);
    }
    ...
    public function FSaMGetDataReport($paDataWhere){
        ...
        if($nPage == $nTotalPage){
            $tRptJoinFooter = " 
                ...
                FROM TRPTPSTaxHDFullTmp WITH(NOLOCK)
        ...
        $tSQL   =   "   
            ...
            FROM (
                SELECT
                    ROW_NUMBER() OVER(ORDER BY FTBchCode ASC, FDXshDocDate ASC) AS RowID,
                    A.*,
                    S.FNRptGroupMember,
                    S.FCXshAmt_SumSup,
                    S.FCXshVat_SumSup,
                    ...
                    S.tBusiness
                FROM TRPTPSTaxHDFullTmp A WITH(NOLOCK)
                /* Calculate Misures */
                LEFT JOIN (
                    SELECT
        ...
        $tSQL .=  " ORDER BY L.FTBchCode ASC, L.FDXshDocDate ASC, LEFT(L.FTXshDocNo,1) DESC ";
    ...
    public function FSnMExecStoreReport($paDataFilter){
        ...
        $tCallStore = "{CALL SP_RPTxPSSVatFull(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
        $aDataStore = array(
            'pnLngID'       => $nLangID , 
            'pnComName'     => $tComName,
            'ptRptCode'     => $tRptCode,
