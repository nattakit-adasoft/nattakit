...
class Rptweeklysale_model extends CI_Model {
    ...
    public function FSnMExecStoreReport($paDataFilter){
        ...
        $tCallStore = "{CALL SP_RPTxPSSaleWeeklyTmp10(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
        $aDataStore = array(
            'pnLngID'       => $nLangID,
            'pnComName'     => $tComName,
            'ptRptCode'     => $tRptCode,
    ...
    public function FSaMGetDataReport($paDataWhere){
        ...
        if($nPage == $nTotalPage){
            $tRptJoinFooter = " SELECT
                                ...
                                FROM TRPTPTTSpcPSSaleWeeklyTmp WITH(NOLOCK)
                                WHERE 1=1
        ...
        $tSQL   =   "   SELECT
                            ...
                            FROM (
                                SELECT
                                    ...
                                    DATA.*,
                                    DTSUMGRP.*
                            FROM TRPTPTTSpcPSSaleWeeklyTmp DATA WITH(NOLOCK)
                            LEFT JOIN (
                                SELECT
    ...
    public function FMaMRPTPagination($paDataWhere){
        ...
        $tSQL = "   
            SELECT
                SPCTMP.FTRptCode
            FROM TRPTPTTSpcPSSaleWeeklyTmp SPCTMP WITH(NOLOCK)
    ...
    public function FMxMRPTSetPriorityGroup($paDataWhere){
        ...
        $tSQL = "   
            UPDATE TRPTPTTSpcPSSaleWeeklyTmp
                SET TRPTPTTSpcPSSaleWeeklyTmp.FNRowPartID = B.PartID
                FROM (
                    SELECT
                        ROW_NUMBER() OVER(PARTITION BY TSPT.FNSaleWeekly ORDER BY TSPT.FNSaleWeekly ASC) AS PartID ,
                        TSPT.FTRptRowSeq
                    FROM TRPTPTTSpcPSSaleWeeklyTmp TSPT WITH(NOLOCK)
            ...
            $tSQL  .= "
                    ...
                    WHERE 1=1
                    AND TRPTPTTSpcPSSaleWeeklyTmp.FTRptRowSeq = B.FTRptRowSeq
                    AND TRPTPTTSpcPSSaleWeeklyTmp.FTComName = '$tComName' 
                    AND TRPTPTTSpcPSSaleWeeklyTmp.FTRptCode = '$tRptCode'
                    AND TRPTPTTSpcPSSaleWeeklyTmp.FTUsrSession = '$tUsrSession' ";
    ...
    public function FSnMCountRowInTemp($paDataWhere) {
        ...
        $tSQL = "   SELECT 
                         ...
                         FROM TRPTPTTSpcPSSaleWeeklyTmp AS TTDT WITH(NOLOCK)
                         WHERE 1 = 1
