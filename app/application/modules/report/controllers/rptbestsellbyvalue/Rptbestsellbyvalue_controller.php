...
date_default_timezone_set("Asia/Bangkok");
class Rptbestsellbyvalue_controller extends MX_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('report/reportbestsellbyvalue/Rptbestsellbyvalue_model');
    ...
    public function index()
    {
        ...
        if (!empty($this->tRptExportType) && !empty($this->tRptCode)) {
            ...
            $this->Rptbestsellbyvalue_model->FSnMExecStoreReport($this->aRptFilter);
    ...
    public function FSvCCallRptViewBeforePrint($aData)
    {
        ...
        // Get Data Report
        $aDataReport = $this->Rptbestsellbyvalue_model->FSaMGetDataReport($aDataWhere); //Get Data
    ...
    public function FSvCCallRptViewBeforePrintClickPage()
    {
        ...
        // Get Data Report
        $aDataReport = $this->Rptbestsellbyvalue_model->FSaMGetDataReport($aDataWhere);
    ...
    public function FCNvCRenderKoolReportHtml($paDataReport, $paDataFilter)
    {
        ...
        require_once APPPATH . 'modules/report/datasources/rptbestsellbyvalue/Rrptbestsellbyvalue_controller.php';

        if (@$aDataReport['rnCurrentPage'] == @$aDataReport['rnAllPage']) {
            // เรียก Summary เฉพาะหน้าสุดท้าย
            @$aSumDataReport = $this->Rptbestsellbyvalue_model->FSaMGetDataSumFootReport($paDataFilter);
        }

        // Set Parameter To Report
        $oRptBestSellHtml = new Rrptbestsellbyvalue_controller(array(
    ...
    public function FSoCChkDataReportInTableTemp($paDataSwitchCase)
    {
        ...
        try {
            ...
            $nDataCountPage = $this->Rptbestsellbyvalue_model->FSnMCountDataReportAll($aDataCountData);
        ...
        if (isset($aDataReport['raItems']) && !empty($aDataReport['raItems'])) {
            ...
            foreach ($aDataReport['raItems'] as $nKey => $aValue) {
                ...
                if (($nKey + 1) == count($aDataReport['raItems'])) { //SumFooter
                    $aSumDataReport = $this->Rptbestsellbyvalue_model->FSaMGetDataSumFootReport($aDataWhere);
                    $values = [
