...
date_default_timezone_set("Asia/Bangkok");
class Rptbestsell_controller extends MX_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('report/reportbestsell/Rptbestsell_model');
    ...
    public function index()
    {
        ...
        if (!empty($this->tRptExportType) && !empty($this->tRptCode)) {
            ...
            $this->Rptbestsell_model->FSnMExecStoreReport($this->aRptFilter);
    ...
    public function FSvCCallRptViewBeforePrint($aData)
    {
        ...
        // Get Data Report
        $aDataReport = $this->Rptbestsell_model->FSaMGetDataReport($aDataWhere); //Get Data
    ...
    public function FSvCCallRptViewBeforePrintClickPage()
    {
        ...
        // Get Data Report
        $aDataReport = $this->Rptbestsell_model->FSaMGetDataReport($aDataWhere);
    ...
    public function FCNvCRenderKoolReportHtml($paDataReport, $paDataFilter)
    {
        ...
        require_once APPPATH . 'modules/report/datasources/rptbestsell/Rrptbestsell_controller.php';

        if (@$aDataReport['rnCurrentPage'] == @$aDataReport['rnAllPage']) {
            // เรียก Summary เฉพาะหน้าสุดท้าย
            @$aSumDataReport = $this->Rptbestsell_model->FSaMGetDataSumFootReport($paDataFilter);
        }

        // Set Parameter To Report
        $oRptBestSellHtml = new Rrptbestsell_controller(array(
    ...
    //         $aDataReport = $this->Rptbestsell_model->FSaMGetDataReport($aDataWhere, $aDataFilter);

    //         //  GetDataSumFootReport
    //         $aDataSumFoot = $this->Rptbestsell_model->FSaMGetDataSumFootReport($aDataWhere, $aDataFilter);
    ...
    public function FSoCChkDataReportInTableTemp($paDataSwitchCase)
    {
        ...
        try {
            ...
            $nDataCountPage = $this->Rptbestsell_model->FSnMCountDataReportAll($aDataCountData);
    ...
    public function FSvCCallRptRenderExcel()
    {
        ...
        // Get Data Report
        $aDataReport = $this->Rptbestsell_model->FSaMGetDataReport($aGetDataReportParams); //Get Data
        ...
        if (isset($aDataReport['raItems']) && !empty($aDataReport['raItems'])) {
            foreach ($aDataReport['raItems'] as $nKey => $aValue) {
                ...
                if (($nKey + 1) == count($aDataReport['raItems'])) { //SumFooter
                    $aDataSumFoot = $this->Rptbestsell_model->FSaMGetDataSumFootReport($aGetDataReportParams, $this->aRptFilter);
                    $values = [
