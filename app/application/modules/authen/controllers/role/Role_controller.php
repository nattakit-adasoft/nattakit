...
class Role_controller extends MX_Controller
{

    public function __construct()
    {
        parent::__construct();
        $this->load->model('authen/role/Role_model');
    ...
    public function FSoCCallPageRoleDataTable()
    {
        try {
            ...
            $aDataRoleList  = $this->Role_model->FSaMGetDataRoleList($aDataWhere);
            $aAlwEvent      = FCNaHCheckAlwFunc('role/0/0');
    ...
    public function FSoCCallPageRoleAdd()
    {
        try {
           ...
           $tSesUsrRoleCodeMulti = $this->session->userdata('tSesUsrRoleCodeMulti');
            ...
            $aDataMenuList = $this->Role_model->FSaMRoleMenuList($aParams);
            $aDataMenuRptList = $this->Role_model->FSaMRptListMenu($aParams);
            $aDataFuncSettingList = $this->Role_model->FSaMGetFuncSettingList($aParams);
    ...
    public function FSoCCallPageRoleEdit()
    {
        try {
            ...
            // Get Data User Role
            $aDataUsrRole = $this->Role_model->FSaMRoleGetDataMaster($aParams);
// ***************************************************************************************
            // Create By Witsarut 190620
            //GetData UsrRoleSpc
            $aDataUsrRoleSpc  = $this->Role_model->FSaMUsrRoleSpcGetDataMaster($aParams);
            ...
            $aDataCountRoleCode = $this->Role_model->FSaMRoleCountNRolCodeFromUsrSpc($aDataAgnCode, $aDataBchCode, $aDataRoleCode);
          
            $aGetDataBranch   = $this->Role_model->FSaMRoleGetBchFromAgnCode($aDataAgnCode);
...
// ***************************************************************************************
            ...
            $aDataMenuList = $this->Role_model->FSaMRoleMenuList($aParams);
            $aDataMenuReport = $this->Role_model->FSaMRptListMenu($aParams);
            $aDataFuncSettingList = $this->Role_model->FSaMGetFuncSettingList($aParams);
            ...
            $aDataRoleMenuEdit = $this->Role_model->FSaMGetDataRoleMenuEdit($aParams);
            $aDataRoleMenuRptEdit = $this->Role_model->FSaMGetDataRoleMenuRptEdit($aParams);
...
// ******************************************************************************************************************************************
            ...
            if($tSpcAgnCode != "" && $tSpcBchCode == ""){
                $aGetBchFromAgnCode = $this->Role_model->FSaMRoleGetBchFromAgnCode($tSpcAgnCode);
                if($aGetBchFromAgnCode['nStaQuery'] == 1){
                    for($i=0; $i < count($aGetBchFromAgnCode['aItems']); $i++){
                        $this->Role_model->FSxMRoleAddUpdateUsrRoleSpc($aGetBchFromAgnCode['aItems'][$i]['FTBchCode'], $aDataMasterRoleSpc);
                    }
                }
            }else{
                if($tSpcAgnCode != '' && $tSpcBchCode != ''){
                    $this->Role_model->FSxMRoleAddUpdateUsrRoleSpc($tSpcBchCode, $aDataMasterRoleSpc);
                }
            }
...
// ********************************************************************************************************************************
            ...
            $this->Role_model->FSxMRoleAddUpdateUsrRole($aDataMaster);
            $this->Role_model->FSxMRoleAddUpdateUsrRoleLang($aDataMaster);

            // Add Data User Menu
            $aRoleMnuData = $this->input->post('paRoleMnuData');
            $this->Role_model->FSxMRoleAddUpdateUsrMenu($aDataMaster, $aRoleMnuData);

            // Add Data Report Menu
            $aRoleRptMnuData = $this->input->post('paRoleRptData');
            $this->Role_model->FSxMRoleAddUpdateUsrRptMenu($aDataMaster, $aRoleRptMnuData);
            ...
            if (!empty($aRoleFuncSettingData)) {
                foreach ($aRoleFuncSettingData as $aRoleFuncSettingItem) {
                    ...
                    $this->Role_model->FSxMAddRoleFuncSetting($aAddRoleFuncSettingParams);
                }
            }else{
                $aClearUsrFuncSettingParams = [
                    "tRoleCode" => $tRoleCode
                ];
                $this->Role_model->FSxClearRoleFuncSetting($aClearUsrFuncSettingParams);
            }
...
// ********************************************************************************************************************************
            ...
            $this->Role_model->FSaMDelRoleCode($tRoleCode);
...
// ********************************************************************************************************************************
    ...
    public function FSoRoleDeleteEvent()
    {
        try {
            ...
            $aStaDelRole    = $this->Role_model->FSaMRoleDeleteData($aDataMaster);
            if ($aStaDelRole['rtCode'] == '1') {
                $nNumRowRolLoc  = $this->Role_model->FSnMCountDataRole();
                $aReturnData    = array(
