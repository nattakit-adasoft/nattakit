...
class Department_controller extends MX_Controller {
    ...
    public function FSvCDPTDataList(){
        try{
            ...
            $aDptDataList   = $this->Department_model->FSaMDPTList($aData); 
    ...
    public function FSvCDPTEditPage(){
        try{
            ...
            $aDptData       = $this->Department_model->FSaMDPTGetDataByID($aData);
            $aDataDpt   = array(
                'nStaAddOrEdit' => 1,
                'aDptData'      => $aDptData
            );
    ...
    public function FSoCDPTAddEvent(){ 
        ...
        try{
          ...
          // Check Auto Gen Department Code?  15/05/2020 Sahatat(Golf)
            ...
            $oCountDup      = $this->Department_model->FSnMDPTCheckDuplicate($aDataDpt['FTDptCode']);
            $nStaDup        = $oCountDup['counts'];
            if($oCountDup !== FALSE && $nStaDup == 0){
                $this->db->trans_begin();
                $aStaDptMaster  = $this->Department_model->FSaMDPTAddUpdateMaster($aDataDpt);
                $aStaDptLang    = $this->Department_model->FSaMDPTAddUpdateLang($aDataDpt);
    ...
    public function FSoCDPTDeleteEvent(){
        ...
        $aResDel        = $this->Department_model->FSaMDPTDelAll($aDataMaster);
        $nNumRowDptLoc  = $this->Department_model->FSnMLOCGetAllNumRow();
