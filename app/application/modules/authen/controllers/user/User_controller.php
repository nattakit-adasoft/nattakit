...
class User_controller extends MX_Controller {
    ...
    public function FSvUSRDataList(){
        try{
            ...
            $tAPIReq    = "";
            $tMethodReq = "GET";
            $aResList = $this->User_model->FSaMUSRList($tAPIReq, $tMethodReq, $aData);
    ...
    public function FSvUSREditPage(){
        try{
            ...
            $tAPIReq        = "";
            $tMethodReq     = "GET";
            $aResList       = $this->User_model->FSaMUSRSearchByID($tAPIReq,$tMethodReq,$aData);

            // Create By Witsarut 21/02/2020
            // Join ขา Edit หา field FTUsrCode
            $aResActRole   = $this->User_model->FSaMActRoleByID($tAPIReq,$tMethodReq,$aData);

            // Create By Witsarut 23/04/2020  (Select only FTBchCode)
            // Last Updated By Napat 11/05/2020 ( ดึงข้อมูล UsrGroup มาทั้งหมด )
            $aResUsrGroup    = $this->User_model->FSaMUsrBchByID($tAPIReq,$tMethodReq,$aData);

            // Create By Witsarut 23/04/2020  (Select only FTShpCode)
            // $aResUsrShp    = $this->User_model->FSaMUsrShpByID($tAPIReq,$tMethodReq,$aData);
    ...
    public function FSoUSRAddEvent(){
        try{
            ...
            //     'FTLastUpdBy'       => $this->session->userdata('tSesUsername')
            // );
            // $this->User_model->FSaMUSRAddUpdateMasterUsrBch($aDataMasterUsrBch);

            $oCountDup  = $this->User_model->FSoMUSRCheckDuplicate($aDataMaster['FTUsrCode']);
            $nStaDup    = $oCountDup[0]->counts;
            if($nStaDup == 0){
                $this->db->trans_begin();

                // TCNMUser
                $aStaUsrMaster      = $this->User_model->FSaMUSRAddUpdateMaster($aDataMaster);
                $aStaUsrLang        = $this->User_model->FSaMUSRAddUpdateLang($aDataMaster);
                // $aStaUsrGroup       = $this->User_model->FSaMUSRAddUpdateGroup($aDataMaster);
                ...
                //     $aGetBchFromMerCode = $this->User_model->FSaMUSRGetBranchFromMerCode($tUsrMerCode);
                //     if($aGetBchFromMerCode['nStaQuery'] == 1){
                //         for($i=0; $i < count($aGetBchFromMerCode['aItems']); $i++){
                //             $this->User_model->FSaMUSRAddUpdateMasterUsrBch($aGetBchFromMerCode['aItems'][$i]['FTBchCode'], $aDataMasterUsrBch);
                //         }
                //     }
                ...
                if($tUsrAgnCode != "" && $tBchCode == ""){
                    ...
                    $this->User_model->FSaMUSRAddUpdateMasterUsrBch('', $aDataMasterUsrBch);
                    // $aGetBchFromAgnCode = $this->User_model->FSaMUSRGetBchFromAgnCode($tUsrAgnCode);
                    // if($aGetBchFromAgnCode['nStaQuery'] == 1){
                    //     for($i=0; $i < count($aGetBchFromAgnCode['aItems']); $i++){
                    //         $this->User_model->FSaMUSRAddUpdateMasterUsrBch($aGetBchFromAgnCode['aItems'][$i]['FTBchCode'], $aDataMasterUsrBch);
                    //     }
                    // }
                ...
                // กรณีเลือกแต่ Merchant ดึง Shop ที่อยู่ภายใต้ Merchant มาทั้งหมด
                    ...
                    $this->User_model->FSaMUSRAddUpdateMasterUsrBch($tBanchCode, $aDataMasterUsrBch);

                    // $aGetShpFromMerCode = $this->User_model->FSaMUSRGetShpFromMerCode($tUsrMerCode);

                    // if($aGetShpFromMerCode['nStaQuery'] == 1){
                    //     for($i=0; $i < count($aGetShpFromMerCode['aItems']); $i++){
                    //         $this->User_model->FSaMUSRAddUpdateMasterUsrShp($aGetShpFromMerCode['aItems'][$i]['FTShpCode'], str_replace(',', '', $tBchCode ), $aDataMasterUsrBch);
                    //     }
                    // }
                }else{
                    if($tBanchCode != '' && $tShpCode == ''){ // Add Usr Bch
                        if(isset($tBranchSpitCode) != empty($tBranchSpitCode)){
                            for($i=0; $i < count($tBranchSpitCode); $i++){
                                ...
                                $aStaMasterUsrBch  = $this->User_model->FSaMUSRAddUpdateMasterUsrBch($tBranchSpitCode[$i], $aDataMasterUsrBch);
                            }
                        }
                        ...
                        if(isset($tShopSpitCode) != empty($tShopSpitCode)){
                            for($i=0; $i < count($tShopSpitCode); $i++){
                                ...
                                $aStaMasterUsrBch  = $this->User_model->FSaMUSRAddUpdateMasterUsrShp($tShopSpitCode[$i], $tBanchCode, $aDataMasterUsrBch);
                            }
                        }
                ...
                if(isset($tRoleSpitCode) != empty($tRoleSpitCode)){
                    for($i=0; $i < count($tRoleSpitCode); $i++){
                        ...
                        $aStaMasterActRole  = $this->User_model->FSaMUSRAddUpdateMasterActRole($tRoleSpitCode[$i],$aDataMasterActRole);
                    }
                }
    ...
    public function FSoUSREditEvent(){
        try{
            ...
            $this->User_model->FSaMDelActRoleCode($aUsrCode);
            ...
            if(isset($tRoleSpitCode) != empty($tRoleSpitCode)){
                for($i=0; $i < count($tRoleSpitCode); $i++){
                    ...
                    $aStaMasterActRole  = $this->User_model->FSaMUpdateMasterActRole($tRoleSpitCode[$i],$aDataMasterActRole);
                }
            }
            ...
            $this->User_model->FSaMDelUsrBchCode($aUsrCode);
            ...
            // กรณีเลือกแต่ Merchant ดึง Shop ที่อยู่ภายใต้ Merchant มาทั้งหมด
                ...
                // Last Update : Napat(Jame) 16/09/2020 ถ้าเลือก Mer ไม่ต้องไปดึง Shp มา
                $this->User_model->FSaMUSRAddUpdateMasterUsrBch($tBchCode, $aDataMasterUsrBch);
                // $aGetShpFromMerCode = $this->User_model->FSaMUSRGetShpFromMerCode($tUsrMerCode);
                // if($aGetShpFromMerCode['nStaQuery'] == 1){
                //     for($i=0; $i < count($aGetShpFromMerCode['aItems']); $i++){
                //         $this->User_model->FSaMUSRAddUpdateMasterUsrShp($aGetShpFromMerCode['aItems'][$i]['FTShpCode'], str_replace(',', '', $tBchCode ), $aDataMasterUsrBch);
                ...
                if($tBchCode != '' && $tShpCode == ''){ // Update Usr Bch
                    if(isset($tBchSpitCode) != empty($tBchSpitCode)){
                        for($i=0; $i < count($tBchSpitCode); $i++){
                            ...
                            $aStaMasterUsrBch  = $this->User_model->FSaMUSRAddUpdateMasterUsrBch($tBchSpitCode[$i], $aDataMasterUsrBch);
                        }
                    }
                    ...
                    if(isset($tShopSpitCode) != empty($tShopSpitCode)){
                        for($i=0; $i < count($tShopSpitCode); $i++){
                            ...
                            $aStaMasterUsrBch  = $this->User_model->FSaMUSRAddUpdateMasterUsrShp($tShopSpitCode[$i], str_replace(',', '', $tBchCode ), $aDataMasterUsrBch);
                        }
                    }
    ...
    public function FSoUSRDeleteEvent(){
        ...
        $aResDel        = $this->User_model->FSnMUSRDel($aDataMaster);
        $nNumRowUsrLoc  = $this->User_model->FSnMLOCGetAllNumRow();
        if($aResDel['rtCode'] == 1){
            $aDeleteImage = array(
                'tModuleName'   => 'authen',
                'tImgFolder'    => 'user',
    ...
    public function FSoUSREventGetRoleUsr(){
        ...
        $aDataUsrRole = $this->User_model->FStUSERGetRoleSpcWhereBrows($tBchCodeUsr);
        $tUsrBchCodeMulti = '';

        if(!empty($aDataUsrRole)){
        $tUsrBchCodeMulti 	= $this->Login_model->FStMLOGMakeArrayToString($aDataUsrRole['aItems'],'FTRolCode','value');
        }
    ...
    public function FSaCUSRGetDataImport(){
		...
		$aGetData 					= $this->User_model->FSaMUSRGetTempData($aDataSearch);
	...
	public function FSaCUSRImportDelete(){
		...
		$aResDel            = $this->User_model->FSaMUSRImportDelete($aDataMaster);
	...
	public function FSaCUSRImportMove2Master(){
		...
		$this->db->trans_begin();
        $this->User_model->FSaMUSRImportMove2Master($aDataMaster);
        $this->User_model->FSaMUSRImportMove2MasterAndReplaceOrInsert($aDataMaster);
        $this->User_model->FSaMUSRImportMove2MasterDeleteTemp($aDataMaster);
    ...
    public function FSaCUSRImportGetItemAll(){
        $aResult  = $this->User_model->FSaMUSRGetTempDataAtAll();
