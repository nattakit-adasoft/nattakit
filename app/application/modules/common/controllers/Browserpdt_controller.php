...
class Browserpdt_controller extends MX_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('common/Browserpdt_model');
    }
    ...
    public function FSxGetProductfotPDT(){
        ...
        $aProduct  = $this->Browserpdt_model->FSnMGetProduct($aDataSearch);
    ...
    public function FSxGetBarcodeforPDT()
    {
        ...
        if ($this->input->post('ptConfig') == '' || $this->input->post('ptConfig') == null) { //printSell
            ...
            $aBarcode       = $this->Browserpdt_model->FSnMGetBarcodePriceSELL($aSendBarcode,$aNotinItem);
            $aCheckPrice    = '';
            $tPriceType     = 'Pricesell';
        } else {
            $tParamSysconfig    = json_decode($this->input->post('ptConfig')); //cost
            //หาประเภทราคา ใช้ต้นทุนแบบไหน [1,2,3,4];
            $aCheckPrice = $this->Browserpdt_model->FSnMGetTypePrice($tParamSysconfig[1], $tParamSysconfig[2], $tParamSysconfig[3]);
            if ($tParamCodeSpl == '' || $tParamCodeSpl == null) { //ไม่ส่ง spl มา
                $aGetInorEx    = $this->Browserpdt_model->FSaMGetWhsInorExIncompany();
                $tVatInorEx    = $aGetInorEx[0]['FTCmpWhsInOrEx'];
            } else {
                $aGetInorEx    = $this->Browserpdt_model->FSaMGetWhsInorExInSupplier($tParamCodeSpl);
                if (empty($aGetInorEx)) {
                    $tVatInorEx    = 1;
                } else {
            ...
            $aBarcode       = $this->Browserpdt_model->FSnMGetBarcodeCOST($aSendBarcode,$aNotinItem);
            $tPriceType     = 'Cost';
        }
