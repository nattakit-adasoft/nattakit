...
class Discountpolicy_controller extends MX_Controller {

    public function __construct() {
        parent::__construct ();
        $this->load->model('settingconfig/discountpolicy/Discountpolicy_model');
    ...
    public function FSvDPCDisGetdataTable(){
        try{
            ...
            $this->Discountpolicy_model->FSaMPRGDataAutoConfig(); 


            // Check Data Status ว่าถ้าเป็น 2 ให้ Delete ออก
            $aGetDataStaUse   = $this->Discountpolicy_model->FSaMPRGDataStaDelUse($aData); 
            ...
            for($k=0; $k<count($aNewArratMerge); $k++){
                if($aNewArratMerge[$k]['USE'] == 'NONE'){
                    ...
                    $this->Discountpolicy_model->FSaMPRGDeleteCodeXY($tDpcDisCodeX, $tDpcDisCodeY);
                }
            }


            // 1. Get ข้อมูลจาก ตาราง TPSTDiscPolicy distinct FTDpcDisCodeX 
            $aResultData  = $this->Discountpolicy_model->FSaMPRGDataComma();

            //Get data Table Header
            $aDatatableHeader   = $this->Discountpolicy_model->FSaMPRGDataTableHeader($aData); 
            ...
            $aDatatable   = $this->Discountpolicy_model->FSaMPRGDataTable($aData, $tDataImplode, $tYColumnShow);
    ...
    public function FSvDPCDisSaveData(){
        try{
            ...
            for($i=0; $i<count($aDataStaAlw); $i++){
                ...
                $aDatatable   = $this->Discountpolicy_model->FSaMPRGUpdateMasterDataTable($aDataUpdate);
            }
        }catch(Exception $Error){
            echo $Error;
        }
